...
//        d($arrQuotations, true);
    ...
    public function getProductVendorSerials($delivery_challan_id, $product_master_id){
        $sql .= " SELECT GROUP_CONCAT(p.`vendor_serial_no`) AS vendor_serial_no, SUM(qty) AS qty";
        $sql .= " FROM ins_delivery_challan_detail dcd";
        $sql .= " INNER JOIN in0_product p";
        $sql .= " ON p.product_id = dcd.product_id";

        $sql .= " WHERE dcd.delivery_challan_id = '{$delivery_challan_id}'";

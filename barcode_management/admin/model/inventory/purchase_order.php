...
class ModelInventoryPurchaseOrder extends HModel {
    ...
    public function getPurchaseOrders($filter, $excl_document_id='') {
        ...
        foreach($purchase_orders as $purchase_order) {
            ...
            // $p = $this->model['product']->getRow(array('product_id' => $purchase_order['product_id']));
            // d($p, true);
            ...
            $arrPurchaseOrders[$purchase_order['document_identity']]['invoice_type'] = $purchase_order['invoice_type'];
            ...
            if(isset($arrPurchaseOrders[$purchase_order['document_identity']]['products'][$purchase_order['purchase_order_detail_id']])) {
                ...
                $arrPurchaseOrders[$purchase_order['document_identity']]['products'][$purchase_order['purchase_order_detail_id']] = array(
                    ...
                    'purchase_order_detail_id' => $purchase_order['purchase_order_detail_id'],
                    ...
                    'tax_percent' => $purchase_order['tax_percent'],
                    'tax_amount' => ($purchase_order['tax_amount']*$purchase_order['conversion_rate']),
                    ...
                    'sub_project_id' => $purchase_order['sub_project_id'],
                    'sub_project' => $purchase_order['sub_project'],
                    'charge_unit_id' => $purchase_order['charge_unit_id'],
                    'charge_unit' => $purchase_order['charge_unit'],
                    'calc_weight' => $purchase_order['calc_weight'],
                    'base_calc_weight' => $purchase_order['base_calc_weight'],
                );
            }
            ...
            $sql .= " WHERE  ref_document_identity='" . $purchase_order['document_identity'] . "'";
            $sql .= " AND purchase_order_detail_id = '".$purchase_order['purchase_order_detail_id']."'";
            if($excl_document_id !='') {
            ...
            $rows = $query->rows;
            foreach($rows as $row) {
                $arrPurchaseOrders[$purchase_order['document_identity']]['products'][$row['purchase_order_detail_id']]['utilized_qty'] += $row['qty'];
            }
        }
        // d($arrPurchaseOrders, true);

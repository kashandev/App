...
class ModelInventoryProductMaster extends HModel {

    protected function getTable() {
        return 'in0_product_master';
    }

    protected function getView() {
        return 'vw_in0_product_master';
    }

    protected function getJoin() {
        $sql  = " FROM `in0_product_master` `p`";
        ...
        $sql .= " LEFT JOIN `in0_make` `mk` ON `mk`.`company_id` = `p`.`company_id` AND `mk`.`make_id` = `p`.`make_id`";
    ...
    public function getMaxProductCode() {
        ...
        $sql .= " FROM `".DB_PREFIX."in0_product_master`";
    ...
    public function getOptionList($search='', $filter=NULL, $sort_order=NULL, $page=0, $limit=25) {
        ...
        $sql = "SELECT p.*, pc.name as product_category, u.name as unit, b.name as brand, mk.name as make, md.name as model";
        $sql .= $this->getJoin();
        if($filter) {
            if(is_array($filter)) {
                $implode = array();
...
//d([$query,$rows],true);
    ...
    public function getProductJson($search, $page, $limit=25, $filter=array()) {
        ...
        $sql = "SELECT count(*) as total_records";
        $sql .= " FROM `vw_in0_product_master`";
        $sql .= " WHERE " . implode(' AND ', $arrWhere);
        $query = $this->conn->query($sql);
        $row = $query->row;
        $total_records = $row['total_records'];

        $sql = "SELECT *, product_master_id as id";
        $sql .= " FROM `vw_in0_product_master`";
        $sql .= " WHERE " . implode(' AND ', $arrWhere);
        $sql .= " LIMIT " . $offset . "," . $limit;
        $query = $this->conn->query($sql);
    ...
    public function getTotalProduct() {
        $sql = "SELECT COUNT(product_id) total_product  FROM `in0_product_master` ";

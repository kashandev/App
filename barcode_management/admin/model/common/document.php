...
class ModelCommonDocument extends HModel {
    ...
    public function getPendingDocuments($filter=array(), $sort_order=array()) {
       ...
       // $sql .= " `l`.`po_no`		          AS `po_no`,";
        ...
        $sql .= " `l`.`project_id`            AS `project_id`,";
        $sql .= " `l`.`sub_project_id`        AS `sub_project_id`,";
        $sql .= " `pro`.`name`        AS `project`,";
        $sql .= " `spro`.`name`        AS `sub_project`,";
        ...
        $sql .= " LEFT JOIN `project` `pro` ON `pro`.`company_id` = `l`.`company_id`  AND `pro`.`project_id` = `l`.`project_id`";
        $sql .= " LEFT JOIN `sub_project` `spro` ON `spro`.`company_id` = `l`.`company_id` AND `spro`.`project_id` = `l`.`project_id` AND `spro`.`sub_project_id` = `l`.`sub_project_id`";

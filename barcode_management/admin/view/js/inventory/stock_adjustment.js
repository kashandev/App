...
//         beforeSend: function() {
//             $('#warehouse_id').before('<i id="loader" class="fas fa-spinner fa-spin"></i>');
//         },
//         complete: function() {
//             $('#loader').remove();
//         },
...
//             if(json.success)
//             {
//                 $('#tblStockAdjustmentDetail tbody').html(json.html);
//             }
//             else {
//                 alert(json.error);
//             }
...
$(document).on('click','#btnAddGrid, .btnAddGrid', function() {
    ...
    $html += '<td>';
    $html += '<input type="hidden" class="form-control" name="stock_adjustment_details['+$grid_row+'][type_of_material]" id="stock_adjustment_detail_type_of_material_'+$grid_row+'" value="" />';
    $html += '<input type="hidden" class="form-control" name="stock_adjustment_details['+$grid_row+'][product_code]" id="stock_adjustment_detail_product_code_'+$grid_row+'" value="" />';
    $html += '<select class="required form-control Select2Product required" id="stock_adjustment_detail_product_id_'+$grid_row+'" name="stock_adjustment_details['+$grid_row+'][product_id]" required>';
    $html += '<option value="">&nbsp;</option>';
    $html += '</select>';
    $html += '<label for="stock_adjustment_detail_product_id_'+$grid_row+'" class="error" style="display:none"></label>';
    $html += '</td>';
    ...
    $html += '<input onchange="calculateWeightTotal(this);" style="min-width: 100px;" type="text" class="form-control fPDecimal" id="stock_adjustment_detail_qty_'+$grid_row+'" value="0" />';
    $html += '</td>';

    $html += '<td>';
    $html += '<input type="text" class="form-control required" id="stock_adjustment_detail_charge_unit_'+$grid_row+'" value="" readonly="true" required/>';
    $html += '<input type="hidden" class="required" id="stock_adjustment_detail_charge_unit_id_'+$grid_row+'" value="" required/>';
    $html += '</td>';

    $html += '<td>';
    $html += '<input type="hidden" id="stock_adjustment_detail_hidden_calc_weight_'+$grid_row+'" value="" />';
    $html += '<input onchange="calculateRowTotal(this);" type="text" class="form-control fPDecimal" id="stock_adjustment_detail_calc_weight_'+$grid_row+'" value="" />';
    $html += '</td>';
    ...
    $('#tblStockAdjustmentDetail tbody').append($html);
    Select2ProductStockAdjustment('#stock_adjustment_detail_product_id_'+$grid_row);
    $('#stock_adjustment_detail_product_id_'+$grid_row).on('select2:select', function (e) {
...
function calculateWeightTotal($obj)
{
    ...
    // Calc Weight
    var $qty = parseFloat($('#stock_adjustment_detail_qty_'+$row_id).val())||0;
    var $cog_weight = parseFloat($('#stock_adjustment_detail_hidden_calc_weight_'+$row_id).val())||0;
    var $calc_weight = $qty*$cog_weight;
    $('#stock_adjustment_detail_calc_weight_'+$row_id).val(rDecimal($calc_weight,4));
...
function calculateRowTotal($obj) {
    ...
    var $calc_weight = parseFloat($('#stock_adjustment_detail_calc_weight_' + $row_id).val())||0;

    var $type_of_material = $('#stock_adjustment_detail_type_of_material_'+$row_id).val();
    ...
    $('#stock_adjustment_detail_amount_' + $row_id).val(roundUpto($amount,2));
...
function calculateTotal() {
    ...
    $('#tblStockAdjustmentDetail tbody tr').each(function() {
...
function getWarehouseStocks($obj){
    ...
    var $data = {
        warehouse_id: $('#warehouse_id').val(),
        product_id: $('#stock_adjustment_detail_product_id_'+$row_id).val(),
        document_date: $('[name="document_date"]').val(),
        document_identity: $('[name="document_identity"]').val(),
        type_of_material: $('#stock_adjustment_detail_type_of_material_' + $row_id).val(),
    };
    ...
    $.ajax({
        ...
        success: function(json) {
            if(json.success)
            {
                $stock_qty = json.stock['stock_qty'] || 0;
                $stock_rate = json.stock['avg_stock_rate'] || 0;
                $stock_amount = json.stock['stock_amount'] || 0;
                $avg_stock_weight = json.stock['avg_stock_weight'] || 0;

                $('#stock_adjustment_detail_stock_qty_'+$row_id).val($stock_qty);

                // Average Stock Qty
                $('#stock_adjustment_detail_qty_'+ $row_id).val(roundUpto($stock_qty, 2));
                $('#stock_adjustment_detail_hidden_qty_'+ $row_id).val($stock_qty);

                // Average Stock Rate
                $('#stock_adjustment_detail_rate_'+ $row_id).val(roundUpto($stock_rate, 2));
                $('#stock_adjustment_detail_hidden_rate_'+ $row_id).val($stock_rate);

                // Average Stock Amount
                $('#stock_adjustment_detail_amount_'+ $row_id).val(roundUpto($stock_amount, 2));
                $('#stock_adjustment_detail_hidden_amount_'+ $row_id).val($stock_amount);

                // Average Stock Weight
                $('#stock_adjustment_detail_hidden_calc_weight_'+ $row_id).val($avg_stock_weight);


                // calculateTotal();
                calculateWeightTotal($obj);
...
function Save() {
    if($('#form').valid() == true){
        ...
        $('.btnSave').attr('disabled','disabled');
        $("#loaderjs").removeClass('hide');
        $('#tblStockAdjustmentDetail tbody tr').each(function(sort_order) {

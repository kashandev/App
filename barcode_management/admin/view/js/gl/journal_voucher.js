...
{
    $project_id = $('#journal_voucher_detail_project_id_' + $row_id).val();
    $sub_project_id = $('#journal_voucher_detail_sub_project_id_' + $row_id).val();
    $.ajax({
        url: $UrlGetJobOrders,
        dataType: 'json',
        type: 'post',
        ...
        success: function(json) {
            if(json.success)
            {
                $('#journal_voucher_detail_job_order_id_'+$row_id).select2({'width':'100%'});
                $('#journal_voucher_detail_job_order_id_'+$row_id).html(json.html);
            }
            else {
                alert(json.error);
            }
...
//    });
//    $html += '</select>';
//    $html += '<label for="journal_voucher_detail_project_id_'+ $grid_row +'" class="error" style="display:none"></label>';
//    $html += '</td>';
//
//    $html += '<td>';
//    $html += '<select onchange="getJobOrders('+$grid_row+')" class="form-control" id="journal_voucher_detail_sub_project_id_'+ $grid_row +'" name="journal_voucher_details['+ $grid_row +'][sub_project_id]">';
//    $html += '</select>';
//    $html += '<label for="journal_voucher_detail_sub_project_id_'+ $grid_row +'" class="error" style="display:none"></label>';
//    $html += '</td>';
//
//    $html += '<td>';
//    $html += '<select class="form-control" id="journal_voucher_detail_job_order_id_'+ $grid_row +'" name="journal_voucher_details['+ $grid_row +'][job_order_id]">';
//    $html += '<option value="">&nbsp;</option>';
//    // $.each($job_orders, function($i, $job_order){
//    //     $html += '<option value="'+$job_order['job_order_id']+'">'+$job_order['document_identity']+'</option>';
//    // });
//    $html += '</select>';
//    $html += '</td>';
...
function setDocumentType($row_id) {
    ...
    $('#journal_voucher_detail_coa_id_' + $row_id).val($obj.coa_id);
    if($obj.adjust_on=='CR') {

...
function getSubProjects($obj)
{
    $row_id = $($obj).parents('tr').data('row_id');
    $project_id = $('#receipt_detail_project_id_' + $row_id).val();
    $.ajax({
        ...
        success: function(json) {
            if(json.success)
            {
                $('#receipt_detail_sub_project_id_'+$row_id).select2('destroy');
                $('#receipt_detail_sub_project_id_'+$row_id).select2({'width':'100%'});
                $('#receipt_detail_sub_project_id_'+$row_id).html(json.html).trigger('change');
            }
            else {
                alert(json.error);
            }
...
function getJobOrders($row_id)
{
    $project_id = $('#receipt_detail_project_id_' + $row_id).val();
    $sub_project_id = $('#receipt_detail_sub_project_id_' + $row_id).val();
    $.ajax({
        url: $UrlGetJobOrders,
        dataType: 'json',
        type: 'post',
        ...
        success: function(json) {
            if(json.success)
            {
                $('#receipt_detail_job_order_id_'+$row_id).select2({'width':'100%'});
                $('#receipt_detail_job_order_id_'+$row_id).html(json.html);
            }
            else {
                alert(json.error);
            }
...
$(document).on('change','#partner_id', function() {
    ...
    $.ajax({
        ...
        mimeType:"multipart/form-data",
        beforeSend: function() {
            $('#ref_document_identity').parent('.input-group').siblings('label').after('&nbsp;<i id="loader" class="fas fa-spinner fa-spin"></i>');
        },
        complete: function() {
            $('#loader').remove();
        },
...
$(document).on('click','#addRefDocument', function() {
    ...
    if( empty($('#ref_document_identity').val()) )
    {
        alert('[Ref. Document No.] Required');
        return;
    }
...
//        $html += '<td style="width: 200px">';
//        $html += '<select onchange="getSubProjects(this)" class="form-control required" id="receipt_detail_project_id_'+ $grid_row +'" name="receipt_details['+$grid_row+'][project_id]" required>';
//        $html += '<option value="">&nbsp;</option>';
//        $.each($projects, function($i, $project){
//            $html += '<option value="'+ $project['project_id'] +'">'+ $project['name'] +'</option>';
//        });
//        $html += '</select>';
//        $html += '<label for="receipt_detail_project_id_'+ $grid_row +'" class="error" style="display:none"></label>';
//        $html += '</td>';
//
//        $html += '<td style="width: 200px">';
//        $html += '<select onchange="getJobOrders('+$grid_row+')" class="form-control required" id="receipt_detail_sub_project_id_'+ $grid_row +'" name="receipt_details['+$grid_row+'][sub_project_id]" required>';
//        $html += '<option value="">&nbsp;</option>';
//        $html += '</select>';
//        $html += '<label for="receipt_detail_sub_project_id_'+ $grid_row +'" class="error" style="display:none"></label>';
//        $html += '</td>';
//
//        $html += '<td style="width: 200px">';
//        $html += '<select class="form-control" id="receipt_detail_job_order_id_'+ $grid_row +'" name="receipt_details['+$grid_row+'][job_order_id]">';
//        $html += '<option value="">&nbsp;</option>';
//        $html += '</select>';
//        $html += '</td>';
        ...
        $html += '<td>';
        $html += '<input type="text" class="form-control fDecimal" name="receipt_details['+$grid_row+'][document_amount]" id="receipt_detail_document_amount_'+$grid_row+'" value="'+roundUpto($document['document_amount'],2)+'" readonly="true"/>';
        $html += '</td>';
        $html += '<td>';
        $html += '<input  type="text" class="form-control fDecimal" id="receipt_detail_balance_amount_'+$grid_row+'" value="'+roundUpto($document['outstanding_amount'],2)+'" readonly="true"/>';
        $html += '</td>';
        $html += '<td>';
        ...
        $html += '</td>';
        $html += '<td>';
        $html += '<input  type="text" class="form-control fDecimal" name="receipt_details['+$grid_row+'][bank_amount]" id="receipt_detail_bank_amount_'+$grid_row+'" value="'+roundUpto($net_amount,2)+'" readonly/>';
        $html += '</td>';
        $html += '<td>';
        ...
        // $html += '</td>'
        $html += '<td>';
        $html += '<input type="text" class="form-control fDecimal" name="receipt_details['+$grid_row+'][net_amount]" id="receipt_detail_net_amount_'+$grid_row+'" value="'+roundUpto($amount,2)+'" readonly="true"/>';
        $html += '</td>';
        $html += '<td>';
...
//    $html += '<td style="width: 200px">';
//    $html += '<select onchange="getSubProjects(this)" class="form-control " id="receipt_detail_project_id_'+ $grid_row +'" name="receipt_details['+$grid_row+'][project_id]" >';
//    $html += '<option value="">&nbsp;</option>';
//    $.each($projects, function($i, $project){
//        $html += '<option value="'+ $project['project_id'] +'">'+ $project['name'] +'</option>';
//    });
...
//
//    $html += '<td style="width: 200px">';
//    $html += '<select onchange="getJobOrders('+$grid_row+')" class="form-control " id="receipt_detail_sub_project_id_'+ $grid_row +'" name="receipt_details['+$grid_row+'][sub_project_id]" >';
//    $html += '<option value="">&nbsp;</option>';
//    $html += '</select>';
//  //  $html += '<label for="receipt_detail_sub_project_id_'+ $grid_row +'" class="error" style="display:none"></label>';
//    $html += '</td>';
...
//    $html += '</td>';
    ...
    $html += '<td>';
    $html += '<input type="text" class="form-control fDecimal" name="receipt_details['+$grid_row+'][document_amount]" id="receipt_detail_document_amount_'+$grid_row+'" value="0.00" readonly="true"/>';
    $html += '</td>';
    $html += '<td>';
    $html += '<input type="text" class="form-control fDecimal" id="receipt_detail_balance_amount_'+$grid_row+'" value="0.00" readonly/>';
    $html += '</td>';
    $html += '<td>';
    ...
    $html += '</td>';
    $html += '<td>';
    $html += '<input  type="text" class="form-control fDecimal" name="receipt_details['+$grid_row+'][bank_amount]" id="receipt_detail_bank_amount_'+$grid_row+'" value="0.00" readonly/>';
    $html += '</td>';
    $html += '<td>';
...
function setBankInfo($row_id)
{
    var $cheque_no = $('#cheque_no').val();
    var $cheque_date = $('#cheque_date').val();
    var $bank_name = $('#bank_name').val();
    $('#receipt_detail_bank_name_' + $row_id).val($bank_name);
    $('#receipt_detail_cheque_date_' + $row_id).val($cheque_date);
    $('#receipt_detail_cheque_no_' + $row_id).val($cheque_no);
}
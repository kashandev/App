...
class ControllerSetupProject extends HController{
        ...
        foreach ($results['lists'] as $aRow) {
            ...
            for ($i = 0; $i < count($aColumns); $i++) {
                if ($aColumns[$i] == 'action') {
                    ...
                    if( strtolower($aRow['status']) == 'active' ) {
                        $row[] = '<span class="d-block p-2 badge badge-success">'. $aRow['status'] .'</span>';
                    } else {
                        $row[] = '<span class="d-block p-2 badge badge-danger">'. $aRow['status'] .'</span>';
                    }
    ...
    protected function validateDelete() {
        ...
        if (!$this->error) {
            ...
            $lang = $this->load->language('setup/project');
            foreach($ids as $id) {
                $countSubProject = $this->model['sub_project']->getCount(array('company_id' => $this->session->data['company_id'], 'project_id' => $id));
               ...
               if($countSubProject > 0) {
                   ...
                   $sub_project = $this->model['sub_project']->getRow(array('company_id' => $this->session->data['company_id'], 'project_id' => $id));
                   $error[] = sprintf($lang['error_delete'], $sub_project['project'], $countSubProject);
                   //d($error);

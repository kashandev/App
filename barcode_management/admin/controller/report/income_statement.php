    ...
    public function printReport() {
        ...
        if($post['output'] == 'Excel'){
            try {
                ...
                //  d($filter,true);
                $rows = $this->model['income_statement']->getIncomeStatement($filter);
...
//                        $pdf->Cell(10, 7,'', 'T', false, 'L', 0, '', 1);
//                        $pdf->Cell(120, 7, 'Total Of '.$sub_group_name, 'T', false, 'L', 0, '', 1);
...
//                    $pdf->Cell(150, 7, 'Total Of '.$group_name, 'T', false, 'L', 0, '', 1);
                ...
                //here + sign is used because total revenue is negative value because income is always on credit side
                //otherwise as per the calculation it should be
                // revenue - expense.
                $deficit = $total_revenue + $total_expense;
                $rowCount++;
                $objPHPExcel->getActiveSheet()->mergeCells('A'.($rowCount).":C".($rowCount));
                $objPHPExcel->getActiveSheet()->SetCellValue('A'.$rowCount, 'Surplus / (deficit) for the year ');
                $objPHPExcel->getActiveSheet()->getStyle('A'.($rowCount).":D".($rowCount))->applyFromArray(
                    array(
                        'borders' => array(
                            'bottom' => array('style' => PHPExcel_Style_Border::BORDER_MEDIUM),
                ...
                $objPHPExcel->getActiveSheet()->SetCellValue('D'.$rowCount, ($deficit));
//                $pdf->Cell(150, 7, 'Surplus / (deficit) for the year ', 'T', false, 'L', 0, '', 1);
//                $pdf->Cell(30, 7, number_format($deficit,2), 'T', false, 'R', 0, '', 1);
...
//                }
                ...
                header('Content-Disposition: attachment;filename="Profit|&|Loss|Report.xls"');
            ...
            foreach($arrRows as $group_name => $group_row )
            {
                ...
                foreach($group_row as $sub_group_name => $rows){
                    ...
                    $pdf->Cell(120, 7, 'Total Of '.$sub_group_name, 'T', false, 'L', 0, '', 1);
                    if($amount < 0){
                        $pdf->Cell(50, 7, number_format($amount*-1,2), 'T', false, 'R', 0, '', 1);
                    }
                    else{
                        $pdf->Cell(50, 7, number_format($amount,2), 'T', false, 'R', 0, '', 1);
                    }

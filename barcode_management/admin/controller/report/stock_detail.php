...
class ControllerReportStockDetail extends HController {

    protected function getAlias() {
        return 'report/stock_detail';
    }
...
//d($row,true);
            ...
            foreach($stocks as $warehouse => $products) {
                foreach($products as $product_name => $rows) {
                    ...
                    foreach($rows as $detail) {
                        ...
                        //$pdf->Cell(35, 7, $detail['batch_no'], 1, false, 'C', 0, '', 1, false, 'M', 'M');
                        $pdf->Cell(35, 7, $detail['product_code'], 1, false, 'C', 0, '', 1, false, 'M', 'M');
                        $pdf->Cell(20, 7, $detail['unit'], 1, false, 'C', 0, '', 1, false, 'M', 'M');
                        $pdf->Cell(20, 7, number_format($detail['qty'],2), 1, false, 'R', 0, '', 1, false, 'M', 'M');
                        $pdf->Cell(30, 7, number_format($detail['rate'],2), 1, false, 'R', 0, '', 1, false, 'M', 'M');
                    ...
                    $pdf->SetFont('helvetica', 'B', 8);
                    $pdf->Ln(7);
                    $pdf->Cell(122, 7, '', 0, false, 'L', 0, '', 1, false, 'M', 'M');
                    $pdf->Cell(20, 7, number_format($balance_qty,2), 1, false, 'R', 0, '', 1, false, 'M', 'M');
    ...
    public function printProductDetailReport() {
        ...
        // d($rows,true);
        foreach($rows as $row) {
            $stocks[$row['product_code'].' - '.$row['product_name']][] = array(
                'container_no' => $row['container_no'],
                'batch_no' => $row['batch_no'],
                'product_code' => $row['product_code'],
                'document_date' => stdDate($row['document_date']),
        ...
        foreach($stocks as $product_name => $details) {
            ...
            foreach($details as $detail) {
                ...
                $pdf->Ln(7);
                $pdf->Cell(7, 7, $sr, 1, false, 'R', 0, '', 1, false, 'M', 'M');
                $pdf->Cell(25, 7, $detail['warehouse'], 1, false, 'C', 0, '', 1, false, 'M', 'M');
                $pdf->Cell(20, 7, $detail['document_date'], 1, false, 'L', 0, '', 1, false, 'M', 'M');
...
//        $this->Cell(0, 10, 'Brand: '.$this->data['brand_id'], 0, false, 'C', 0, '', 0, false, 'M', 'M');
        ...
        if($this->data['print'] == 'warehouseDetail')
        {
            ...
            $this->Cell(30, 7, 'Rate', 1, false, 'C', 0, '', 0, false, 'M', 'M');

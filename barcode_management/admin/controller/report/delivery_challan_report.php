...
//
    ...
    public function printReportExcel()
    {
        ...
        if($product_id != ''){
            $where .= " AND `product_master_id`='$product_id'";
        }
        ...
        $objPHPExcel->getActiveSheet()->SetCellValue('C'.$rowCount, 'Serial No')->getStyle('C'.$rowCount)->getFont()->setBold( true );
        $objPHPExcel->getActiveSheet()->SetCellValue('D'.$rowCount, 'Batch No')->getStyle('D'.$rowCount)->getFont()->setBold( true );
        $objPHPExcel->getActiveSheet()->SetCellValue('E'.$rowCount, 'Warehouse Name')->getStyle('E'.$rowCount)->getFont()->setBold( true );
        $objPHPExcel->getActiveSheet()->SetCellValue('F'.$rowCount, 'Partner Name')->getStyle('F'.$rowCount)->getFont()->setBold( true );
        $objPHPExcel->getActiveSheet()->SetCellValue('G'.$rowCount, 'Product')->getStyle('G'.$rowCount)->getFont()->setBold( true );
        $objPHPExcel->getActiveSheet()->SetCellValue('H'.$rowCount, 'PO No.')->getStyle('H'.$rowCount)->getFont()->setBold( true );
        $objPHPExcel->getActiveSheet()->SetCellValue('I'.$rowCount, 'Unit')->getStyle('I'.$rowCount)->getFont()->setBold( true );
        $objPHPExcel->getActiveSheet()->SetCellValue('J'.$rowCount, 'Quantity')->getStyle('J'.$rowCount)->getFont()->setBold( true );
        $objPHPExcel->getActiveSheet()->SetCellValue('K'.$rowCount, 'Rate')->getStyle('K'.$rowCount)->getFont()->setBold( true );
        $objPHPExcel->getActiveSheet()->SetCellValue('L'.$rowCount, 'Amount')->getStyle('L'.$rowCount)->getFont()->setBold( true );
        ...
        foreach($arrRows as $key => $value) {
            $objPHPExcel->getActiveSheet()->mergeCells('A'.($rowCount).":L".($rowCount));
            $objPHPExcel->getActiveSheet()->SetCellValue('A'.$rowCount, 'Group By: ' . $key);
            $objPHPExcel->getActiveSheet()->getStyle('A'.$rowCount)->getFill('ebebeb');
            $rowCount ++;
            ...
            foreach ($value as $key_1 => $value_1)
            {
                ...
                $objPHPExcel->getActiveSheet()->SetCellValue('C'.$rowCount, $value_1['serial_no']);
                $objPHPExcel->getActiveSheet()->SetCellValue('D'.$rowCount, $value_1['batch_no']);
                $objPHPExcel->getActiveSheet()->SetCellValue('E'.$rowCount, $value_1['warehouse_name']);
                $objPHPExcel->getActiveSheet()->SetCellValue('F'.$rowCount, $value_1['partner_name']);
                $objPHPExcel->getActiveSheet()->SetCellValue('G'.$rowCount, $value_1['product_name']);
                $objPHPExcel->getActiveSheet()->SetCellValue('H'.$rowCount, $value_1['po_no']);
                $objPHPExcel->getActiveSheet()->SetCellValue('I'.$rowCount, $value_1['unit_name']);
                $objPHPExcel->getActiveSheet()->SetCellValue('J'.$rowCount, $value_1['qty']);
                $objPHPExcel->getActiveSheet()->SetCellValue('K'.$rowCount, $value_1['cog_rate']);
                $objPHPExcel->getActiveSheet()->SetCellValue('L'.$rowCount, $value_1['cog_amount']);
                $rowCount++;

                $total_qty += $value_1['qty'];
                $total_amount += $value_1['cog_amount'];

            }
            $rowCount++;

            $objPHPExcel->getActiveSheet()->SetCellValue('J'.$rowCount, $total_qty);
            $objPHPExcel->getActiveSheet()->SetCellValue('L'.$rowCount, $total_amount);
            $objPHPExcel->getActiveSheet()->getStyle('A'.$rowCount.':L'.$rowCount)->applyFromArray(
                array(
                    'alignment' => array(
...
//        d($where,true);
        ...
        $company = $this->model['company']->getRow(array('company_id' => $this->session->data['company_id']));
        $rows = $this->model['delivery_challan_detail']->getRows($where,array('document_date asc', 'serial_no asc'));
       // d($rows, true);
...
//        d($arrRows,true);
        foreach($arrRows as $group_name => $rows)
        {
            ...
            foreach($rows as $delivery_challans) {
                ...
                $pdf->Cell(22, 5, stdDate($delivery_challans['document_date']), 0, false, 'C', 0, '', 1);
                $pdf->Cell(22, 5, html_entity_decode($delivery_challans['document_identity']), 0, false, 'C', 0, '', 1);
                $pdf->Cell(25, 5, $delivery_challans['serial_no'], 0, false, 'C', 0, '', 1);
                $pdf->Cell(25, 5, $delivery_challans['batch_no'], 0, false, 'C', 0, '', 1);
                $pdf->Cell(30, 5, $delivery_challans['warehouse_name'], 0, false, 'L', 0, '', 1);
                $pdf->Cell(45, 5, $delivery_challans['partner_name'], 0, false, 'L', 0, '', 1);
                $pdf->Cell(55, 5, $delivery_challans['product_name'], 0, false, 'L', 0, '', 1);
                $pdf->Cell(20, 5, $delivery_challans['po_no'], 0, false, 'C', 0, '', 1);
                $pdf->Cell(15, 5, $delivery_challans['unit_name'], 0, false, 'C', 0, '', 1);
                $pdf->Cell(25, 5, number_format($delivery_challans['qty'],2), 0, false, 'R', 0, '', 1);
//                $pdf->Cell(22, 5, number_format($delivery_challans['cog_rate'],2), 0, false, 'R', 0, '', 1);
//                $pdf->Cell(25, 5, number_format($delivery_challans['cog_amount'],2), 0, false, 'R', 0, '', 1);
        ...
        $pdf->Cell(259, 5, '', 0, false, 'R', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(25,  5, number_format($total_quantity,2), 'T,B,B', false, 'R', 0, '', 0, false, 'M', 'M');
//        $pdf->Cell(22,  5, '', 0, false, 'R', 0, '', 0, false, 'M', 'M');
//        $pdf->Cell(25,  5, number_format($total_amount,2), 'T,B,B', false, 'R', 0, '', 0, false, 'M', 'M');
        //Close and output PDF document
        $pdf->Output('Delivery Challan Report:'.date('YmdHis').'.pdf', 'I');
...
class PDF extends TCPDF {
    ...
    public function Header() {
        ...
        $this->Cell(22, 5, 'Doc Date', 'T,B', false, 'C', 0, '', 0, false, 'M', 'M');
        $this->Cell(22, 5, 'Doc No.', 'T,B', false, 'C', 0, '', 0, false, 'M', 'M');
        $this->Cell(25, 5, 'Serial No', 'T,B', false, 'C', 0, '', 0, false, 'M', 'M');
        $this->Cell(25, 5, 'Batch No', 'T,B', false, 'C', 0, '', 0, false, 'M', 'M');
        $this->Cell(30, 5, 'Warehouse', 'T,B', false, 'C', 0, '', 0, false, 'M', 'M');
        $this->Cell(45, 5, 'Partner', 'T,B', false, 'C', 0, '', 0, false, 'M', 'M');
        $this->Cell(55, 5, 'Product', 'T,B', false, 'C', 0, '', 0, false, 'M', 'M');
        $this->Cell(20, 5, 'Po No', 'T,B', false, 'C', 0, '', 0, false, 'M', 'M');
        $this->Cell(15, 5, 'Unit', 'T,B', false, 'C', 0, '', 0, false, 'M', 'M');
        $this->Cell(25, 5, 'Quantity', 'T,B', false, 'C', 0, '', 0, false, 'M', 'M');
//        $this->Cell(22, 5, 'Rate', 'T,B', false, 'C', 0, '', 0, false, 'M', 'M');
//        $this->Cell(25, 5, 'Amount', 'T,B', false, 'C', 0, '', 0, false, 'M', 'M');

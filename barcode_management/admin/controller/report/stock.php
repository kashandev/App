...
class ControllerReportStock extends HController {
    ...
    protected function getList() {
        ...
        $this->data['report_type_product_report'] = $this->url->link($this->getAlias() .'/printProductDetailReport', 'token=' . $this->session->data['token'], 'SSL');
    ...
    public function getProductJson() {
        ...
        $rows = $this->model['product_master']->getProductJson($search, $page, 25, array('product_category_id' => $product_category_id));
    ...
    public function getDetailReport() {
        ...
        $rows = $this->model['stock']->getRows($where, array('document_date'));
        // d($rows, true);
        $html = '';
        foreach($rows as $row) {
            $html .= '<tr>';
        ...
        if($post['output'] == 'Excel'){
            ...
            foreach($invoices as $row) {
                ...
                foreach($row['data'] as $detail) {
                    ...
                    $objPHPExcel->getActiveSheet()->setCellValue('D'.$rowcount,($detail['product_code'].' - '.$detail['product_name']))->getStyle('D'.$rowcount)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
...
//d($row,true);
    ...
    public function printWarehouseDetail() {
        ...
        if($post['product_id']) {
            $arrWhere[] = "`product_id` = '" . $post['product_id'] . "'";
            $filter['product'] = $this->model['product']->getArrays('product_id','name',array('company_id' => $this->session->data['company_id'], 'product_id' => $post['product_id']));
        }
        ...
        $where = implode(' AND ', $arrWhere);
        // $rows = $this->model['stock']->getRows($where, array('document_date', 'created_at'));
        $rows = $this->model['stock']->getRows($where, array('`document_date`'));
        // d($rows,true);
        $stocks = array();
        $this->model['product_master'] = $this->load->model('inventory/product_master');
        foreach($rows as $row) {
            ...
            $product_master = $this->model['product_master']->getRow(array('product_master_id' => $row['product_id']));
            // d([$row,$product_master], true);

            $stocks[$row['warehouse']][$row['product_code'] . ' - ' . $product_master['model'] . ' - ' . $row['product_name']][] = array(
                'container_no' => $row['container_no'],
                'batch_no' => $row['batch_no'],
                'product_code' => $row['product_code'],
                'document_date' => stdDate($row['document_date']),
        ...
        if($post['output'] == 'Excel'){
            ...
            foreach($stocks as $warehouse => $products) {
                foreach($products as $product_name => $rows) {
                    ...
                    $pdf->ln(5);
                    $pdf->SetFillColor(255,255,255);
                    $pdf->Cell(100, 7,'Product: ' . $product_name, 0, false, 'L', 1, '', 0, false, 'M', 'M');
                    $pdf->ln(5);
                    $pdf->Cell(77, 7,'Warehouse: ' . $warehouse, 0, false, 'L', 1, '', 0, false, 'M', 'M');
                    // $pdf->Ln(7);
                    $sr =0;
                    $balance_qty = 0;
                    $balance_amount = 0;
    ...
    public function printContainerDetail() {
        ...
        $where = implode(' AND ', $arrWhere);
        // $rows = $this->model['stock']->getRows($where, array('document_date', 'created_at'));
        $rows = $this->model['stock']->getRows($where, array('`document_date`', 'sfd_sort_order'));
        // d($rows,true);
        $stocks = array();
        foreach($rows as $row) {
            $stocks[$row['container_no'] . '     Brand : '. $row['brand']][$row['product_code'] . ' - ' . $row['product_name']][] = array(
                'container_no' => $row['container_no'],
                'batch_no' => $row['batch_no'],
                'product_code' => $row['product_code'],
                'document_date' => stdDate($row['document_date']),
    ...
    public function printWarehouseSummary() {
        ...
        foreach($rows as $row) {
            ...
            if(!isset($stocks[$row['warehouse']][$row['product_id']])) {
                $stocks[$row['warehouse']][$row['product_id']] = array(
                    'warehouse' => $row['warehouse'],
                    'product_code' => $row['product_code'],
                    'product_name' => $product_master['model'].' - '.$row['product_name'],
                    'total_qty' => 0,
                    'total_amount' => 0,
                    'unit' => $row['document_unit'],
                );
        ...
        if($post['output'] == 'Excel'){
            ...
            foreach($stocks as $warehouse => $products) {
                ...
                $objPHPExcel->getActiveSheet()->setCellValue('B'.$rowcount, 'Code')->getStyle('B'.$rowcount)->getFont()->setBold(true);
                $objPHPExcel->getActiveSheet()->setCellValue('C'.$rowcount, 'Product')->getStyle('B'.$rowcount)->getFont()->setBold(true);
                $objPHPExcel->getActiveSheet()->setCellValue('D'.$rowcount, 'Quantity')->getStyle('C'.$rowcount)->getFont()->setBold(true);
                $objPHPExcel->getActiveSheet()->setCellValue('E'.$rowcount, 'Amount')->getStyle('D'.$rowcount)->getFont()->setBold(true);
                $objPHPExcel->getActiveSheet()->setCellValue('F'.$rowcount, 'Avg Cost')->getStyle('E'.$rowcount)->getFont()->setBold(true);
                ...
                foreach($products as $detail) {
                    if($detail['total_qty'] != 0) {
                        ...
                        $objPHPExcel->getActiveSheet()->setCellValue('B'.$rowcount,$detail['product_code'])->getStyle('B'.$rowcount)->getAlignment(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
                        $objPHPExcel->getActiveSheet()->setCellValue('C'.$rowcount,$detail['product_name'])->getStyle('B'.$rowcount)->getAlignment(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
                        $objPHPExcel->getActiveSheet()->setCellValue('D'.$rowcount,$detail['total_qty'])->getStyle('C'.$rowcount)->getAlignment(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
                        $objPHPExcel->getActiveSheet()->setCellValue('E'.$rowcount,$detail['total_amount'])->getStyle('D'.$rowcount)->getAlignment(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
                        $objPHPExcel->getActiveSheet()->setCellValue('F'.$rowcount,$detail['total_amount']/$detail['total_qty'])->getStyle('E'.$rowcount)->getAlignment(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
                        $objPHPExcel->getActiveSheet()->getStyle('A'.$rowcount.':F'.$rowcount)->applyFromArray(
                            array(
                                'borders' => array(
                                    'allborders' => array('style' => PHPExcel_Style_Border::BORDER_MEDIUM),
                ...
                $rowcount++;
                $objPHPExcel->getActiveSheet()->setCellValue('D'.$rowcount,$balance_qty)->getStyle('C'.$rowcount)->getFont()->setBold(true);
                $objPHPExcel->getActiveSheet()->setCellValue('E'.$rowcount,$balance_amount)->getStyle('F'.$rowcount)->getFont()->setBold(true);
                $objPHPExcel->getActiveSheet()->getStyle('A'.$rowcount.':F'.$rowcount)->applyFromArray(
                    array(
                        'fill' => array(
                            'type' => PHPExcel_Style_Fill::FILL_SOLID,
            ...
            foreach($stocks as $warehouse => $products) {
                ...
                foreach($products as $detail) {
                    if($detail['total_qty'] != 0) {
                        ...
                        $pdf->Cell(25, 7, number_format(($detail['total_amount']/$detail['total_qty']),2), 1, false, 'R', 0, '', 1, false, 'M', 'M');
                    }
                }
                // set font
                $pdf->SetFont('helvetica', 'B', 8);
    ...
    public function printProductDetailReport() {
        ...
        foreach($rows as $row) {
            ...
            $stocks[$row['product_code'].' - '.$product_master['model'].' - '.$row['product_name']][] = array(
                'container_no' => $row['container_no'],
                'batch_no' => $row['batch_no'],
                'product_code' => $row['product_code'],
                'document_date' => stdDate($row['document_date']),
        ...
        foreach($stocks as $product_name => $details) {
            $pdf->SetFont('times', 'B', 9);
            $pdf->ln(10);
            $pdf->SetFillColor(255,255,255);
            $pdf->Cell(77, 7, $product_name, 0, false, 'L', 1, '', 0, false, 'M', 'M');
            $pdf->Ln(10);
            $pdf->SetFont('times', 'B', 8);
            $pdf->Cell(7, 10, 'Sr.', 1, false, 'C', 0, '', 0, false, 'M', 'M');
            $pdf->Cell(25, 10, $lang['warehouse'], 1, false, 'C', 0, '', 0, false, 'M', 'M');
            $pdf->Cell(20, 10, $lang['document_date'], 1, false, 'C', 0, '', 0, false, 'M', 'M');
            $pdf->Cell(25, 10, $lang['document_no'], 1, false, 'C', 0, '', 0, false, 'M', 'M');
            ...
            $pdf->Cell(23, 10, 'Stock Evaluation', 1, false, 'C', 0, '', 0, false, 'M', 'M');
            $pdf->Cell(23, 10, 'Balance Qty', 1, false, 'C', 0, '', 0, false, 'M', 'M');
            $pdf->Cell(17, 10, 'Avg Cost', 1, false, 'C', 0, '', 0, false, 'M', 'M');
            $pdf->Ln(2);
            $sr =0;
            $balance_qty = 0;
            $balance_amount = 0;
            ...
            foreach($details as $detail) {
                ...
                $pdf->Ln(7);
                $pdf->Cell(7, 7, $sr, 1, false, 'R', 0, '', 1, false, 'M', 'M');
                $pdf->Cell(25, 7, $detail['warehouse'], 1, false, 'R', 0, '', 1, false, 'M', 'M');
                $pdf->Cell(20, 7, $detail['document_date'], 1, false, 'L', 0, '', 1, false, 'M', 'M');
                $pdf->Cell(25, 7, $detail['document_identity'], 1, false, 'L', 0, '', 1, false, 'M', 'M');
                ...
                $pdf->Cell(25, 7, number_format($detail['amount'],2), 1, false, 'R', 0, '', 1, false, 'M', 'M');
                $pdf->Cell(23, 7, number_format($balance_amount,2), 1, false, 'C', 0, '', 1, false, 'M', 'M');
                $pdf->Cell(23, 7, number_format($balance_qty,2), 1, false, 'C', 0, '', 1, false, 'M', 'M');
                $pdf->Cell(17, 7, number_format($avg_rate,2), 1, false, 'R', 0, '', 1, false, 'M', 'M');
            }
            $pdf->SetFont('times', 'B', 8);
            $pdf->Ln(7);
            $pdf->Cell(77, 7, '', 0, false, 'L', 0, '', 1, false, 'M', 'M');
            $pdf->Cell(15, 7, number_format($balance_qty,2), 1, false, 'R', 0, '', 1, false, 'M', 'M');
            $pdf->Cell(20, 7, '', 1, false, 'R', 0, '', 1, false, 'M', 'M');
            $pdf->Cell(25,   7, number_format($balance_amount,2), 1, false, 'R', 0, '', 1, false, 'M', 'M');
            $pdf->ln(9);

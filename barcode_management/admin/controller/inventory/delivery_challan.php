...
class ControllerInventoryDeliveryChallan extends HController {
    ...
    public function getAjaxLists() {
        ...
        $data = array();
        $aColumns = array('action', 'document_date', 'document_identity', 'partner_name', 'total_qty', 'challan_status','invoice_status', 'created_at', 'check_box');
        ...
        foreach ($results['lists'] as $aRow) {
            ...
            $actions[] = array(
                'text' => 'Without Header',
                'target' => '_blank',
                'href' => $this->url->link($this->getAlias() . '/printDocument', 'token=' . $this->session->data['token'] . '&' . $this->getPrimaryKey() . '=' . $aRow[$this->getPrimaryKey()] . '&header=2', 'SSL'),
                'btn_class' => 'btn btn-info btn-xs',
                'class' => 'fa fa-print'
            );
...
//                );
    ...
    protected function getForm() {
        ...
        $this->data['allow_out_of_stock'] = $status['value'] ? 0 :0;
...
//        $this->data['href_get_sale_order'] =  $this->url->link($this->getAlias() . '/getData', 'token=' . $this->session->data['token'] . '&' . $this->getPrimaryKey() . '=' . $this->request->get[$this->getPrimaryKey()], 'SSL');
        ...
        if (isset($this->request->get['delivery_challan_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
            ...
            foreach($rows as $row_no => $row) {
                ...
                $sale_order = $this->model['sale_order']->getRow(array('document_identity' => $row['ref_document_identity']));
                $this->model['stock'] = $this->load->model('common/stock_ledger');
                $stock = $this->model['stock']->getWarehouseStock($row['product_id'], $row['warehouse_id'], $result['document_identity'], $result['document_date']);
                $this->data['delivery_challan_details'][$row_no] = $row;
                $this->data['delivery_challan_details'][$row_no]['stock_qty']         = $stock['stock_qty'];
                $this->data['delivery_challan_details'][$row_no]['avg_stock_rate']    = $stock['avg_stock_rate'];
                $this->data['delivery_challan_details'][$row_no]['stock_amount']      = ($stock['stock_qty'] * $stock['avg_stock_rate']);
                $this->data['delivery_challan_details'][$row_no]['href'] = $this->url->link('inventory/sale_order/update', 'token=' . $this->session->data['token'] . '&sale_order_id=' .$sale_order['sale_order_id'], 'SSL');
...
// d($this->data['delivery_challan_details'],true);
        ...
        $this->data['action_print_without_header'] = $this->url->link($this->getAlias() . '/printDocument', 'token=' . $this->session->data['token'] . '&' . $this->getPrimaryKey() . '=' . $this->request->get[$this->getPrimaryKey()].'&header=2', 'SSL');
    ...
    public function getProductJson() {
        ...
        $filter[] = "p.product_type = 'Finished Goods'";
       ...
       // d('In',true);
        ...
        foreach($sale_order['products'] as $product) {
            if($product['order_qty'] - $product['utilized_qty'] > 0)
            {
                ...
                $Cog_total = $balanceQty * $sale_order['rate'];

            $arrProduct[$product['product_master_id']]=$balanceQty;


                $html .= '<tr id="grid_row_'.$row_no.'" data-row_id="'.$row_no.'">';

                $html .= '<td>';
                $html .= '<a title="Remove" class="btnRemoveGrid btn btn-xs btn-danger" href="javascript:void(0);"><i class="fa fa-times"></i></a>';
                ...
                $html .= '<td>';
                $html .= '<input type="hidden" class="form-control" id="delivery_challan_detail_product_master_id_'.$row_no.'" name="delivery_challan_details['.$row_no.'][product_master_id]" value="'.$product['product_master_id'].'">';
                $html .= '<input type="text" class="form-control" id="delivery_challan_detail_serial_no_'.$row_no.'" name="delivery_challan_details['.$row_no.'][serial_no]" value="'.$product['serial_no'].'" readonly>';
                $html .= '</td>';

                $html .= '<td>';
                $html .= '<input type="hidden" class="form-control" id="delivery_challan_detail_product_code_'.$row_no.'" name="delivery_challan_details['.$row_no.'][product_code]" value="'.$product['product_code'].'"  />';
                $html .= '<input type="hidden" class="form-control" id="delivery_challan_detail_product_id_'.$row_no.'" name="delivery_challan_details['.$row_no.'][product_id]" value="'.$product['product_id'].'">';
                $html .= '<input type="hidden" class="form-control" id="delivery_challan_detail'.$row_no.'_product_master_id" name="delivery_challan_details['.$row_no.'][product_master_id]" value="'.$product['product_master_id'].'">';

                $html .= '<input type="text" class="form-control" id="delivery_challan_detail_product_name_'.$row_no.'" name="delivery_challan_details['.$row_no.'][product_name]" value="'.$product['product_code'].' - '.html_entity_decode($product['product_name']).'" readonly>';
                $html .= '</td>';

                $html .= '<td>';
                $html .= '<input type="text" class="form-control" id="delivery_challan_detail_description_'.$row_no.'" name="delivery_challan_details['.$row_no.'][description]" value="'.htmlentities($product['description']).'" readonly /></td>';

                $html .= '<td>';
                $html .= '<input type="text" class="form-control" id="delivery_challan_detail_batch_no_'.$row_no.'" name="delivery_challan_details['.$row_no.'][batch_no]" value="'.htmlentities($product['batch_no']).'" readonly /></td>';

                $html .= '<td>';
                $html .= '<select onchange="getWarehouseStock(this);" type="text" class="form-control select2 warehouse" id="delivery_challan_detail_warehouse_id_'.$row_no.'" name="delivery_challan_details['.$row_no.'][warehouse_id]" required>';
                $html .= ' <option value="">&nbsp;</option>';
                foreach($warehouses as $warehouse){
                    $html .= '<option value="'.$warehouse['warehouse_id'].'">'.$warehouse['name'].'</option>';
                }
                $html .= '</select>';
                $html .= '<label for="delivery_challan_detail_warehouse_id_'.$row_no.'" class="required" style="display:none"></label>';
                $html .= '</td>';
                $html .= '<td>';
                $html .= '<input type="text" onchange="calculateRowTotal(this)" class="form-control" id="delivery_challan_detail_qty_'.$row_no.'" name="delivery_challan_details['.$row_no.'][qty]" value="'.$balanceQty.'" >';
                $html .= '<input type="hidden" class="form-control" id="delivery_challan_detail_availabel_stock_'.$row_no.'" name="delivery_challan_details['.$row_no.'][availabel_stock]" value="" >';
                $html .= '</td>';
                $html .= '<td><input type="text" class="form-control" id="delivery_challan_detail_unit_'.$row_no.'" name="delivery_challan_details['.$row_no.'][unit]" value="'.$product['unit'].'" readonly>';
                $html .= '<input type="hidden" class="form-control" id="delivery_challan_detail_unit_id_'.$row_no.'" name="delivery_challan_details['.$row_no.'][unit_id]" value="'.$product['unit_id'].'"></td>';
                $html .= '<input type="hidden" class="form-control" id="delivery_challan_detail_rate_'.$row_no.'" name="delivery_challan_details['.$row_no.'][rate]" value="'.$product['rate'].'" readonly>';
                $html .= '<input type="hidden" class="form-control" id="delivery_challan_detail_cog_rate_'.$row_no.'" name="delivery_challan_details['.$row_no.'][cog_rate]" value="'.$product['cog_rate'].'" readonly>';
                $html .= '<input type="hidden" class="form-control" id="delivery_challan_detail_cog_amount_'.$row_no.'" name="delivery_challan_details['.$row_no.'][cog_amount]" value="'.$product['cog_amount'].'" />';

                $html .= '<input type="hidden" class="form-control" id="delivery_challan_detail_tax_percent_'.$row_no.'" name="delivery_challan_details['.$row_no.'][tax_percent]" value="'.$product['tax_percent'].'" />';
                $html .= '<input type="hidden" class="form-control" id="delivery_challan_detail_tax_amount_'.$row_no.'" name="delivery_challan_details['.$row_no.'][tax_amount]" value="'.$product['tax_amount'].'" />';
                $html .= '<td>';
                $html .= '<a title="Remove" class="btnRemoveGrid btn btn-xs btn-danger" href="javascript:void(0);"><i class="fa fa-times"></i></a>';
                $html .= '<a title="Add" class="btn btn-xs btn-primary btnAddGrid" id="btnAddGrid" href="javascript:void(0);"><i class="fa fa-plus"></i></a></td>';
                $html .= '</tr>';
...
//        d($sale_orders,true);
        ...
        foreach($sale_orders as $sale_order){
            ...
            // $html .= '<input type="hidden" class="form-control" id="delivery_challan_detail'.$id.'_product_code" name="delivery_challan['.$id.'][product_code]" value="'.$sale_order['product_code'].'" readonly />';
            $html .= '</td>';
            $html .= '<td>';
            $html .= '<input type="hidden" class="form-control" id="delivery_challan_detail'.$id.'_product_id" name="delivery_challan_details['.$id.'][product_id]" value="'.$sale_order['product_id'].'">';
            $html .= '<input type="hidden" class="form-control" id="delivery_challan_detail'.$id.'_product_master_id" name="delivery_challan_details['.$id.'][product_master_id]" value="'.$sale_order['product_master_id'].'">';
            ...
            $html .= '<td>';
            $html .= '<select type="text" class="form-control" id="delivery_challan_detail'.$id.'_warehouse_id" name="delivery_challan_details['.$id.'][warehouse_id]" >';
            $html .= ' <option value="">&nbsp;</option>';
            foreach($warehouses as $warehouse){
                $html .= '<option value="'.$warehouse['warehouse_id'].'">'.$warehouse['name'].'</option>';
            }
...
//d($json,true);
    ...
    protected function insertData($data) {
        ...
        $data['delivery_challan_details'] = $this->session->data['detail'][$form_key];
        ...
        foreach ($data['delivery_challan_details'] as $sort_order => $detail) {
            ...
            $this->model['stock'] = $this->load->model('common/stock_ledger');
            $stock = $this->model['stock']->getWarehouseStock($detail['product_master_id'], $detail['warehouse_id'], $data['document_identity'], $data['document_date']);
           // d([$detail,$product,$stock],true);
    ...
    protected function updateData($primary_key, $data) {
        ...
        $this->model['document']->deleteBulk($this->getAlias(), array('document_type_id' => $this->document_type_id,'document_id' => $delivery_challan_id));
        $this->model['ledger']->deleteBulk($this->getAlias(), array('document_type_id' => $this->document_type_id,'document_id' => $delivery_challan_id));
        $this->model['stock_ledger']->deleteBulk($this->getAlias(), array('document_type_id' => $this->document_type_id, 'document_id' => $delivery_challan_id));
        $this->model['stock_ledger_history']->deleteBulk($this->getAlias(), array('document_type_id' => $this->document_type_id,'document_id' => $delivery_challan_id));
...
//        $this->data['company_footer'] = $this->model['image']->resize('footer.jpeg',200,50);
        ...
        $pdf->Cell(35, 7, $DeliveryChallan['document_identity'], 'B', false, 'L', 0, '', 0, false, 'M', 'M');

        $pdf->Cell(75, 7, '', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(5, 7, '', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        
        $pdf->Cell(20, 7, 'Doc Date', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(40, 7, stdDate($DeliveryChallan['document_date']), 'B', false, 'L', 0, '', 0, false, 'M', 'M');
        ...
        $pdf->Ln(7);
        $pdf->Cell(20, 7, 'Order By', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(110, 7, $DeliveryChallan['order_by'], 'B', false, 'L', 0, '', 0, false, 'M', 'M');

        $pdf->Cell(5, 7, '', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        
        $pdf->Cell(20, 7, 'Vehicle No.', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(40, 7, $DeliveryChallan['vehicle_no'], 'B', false, 'L', 0, '', 0, false, 'M', 'M');

        $pdf->Ln(7);
        $pdf->Cell(20, 7, 'Cargo Name', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(70, 7, $DeliveryChallan['cargo_name'], 'B', false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(5, 7, '', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(15, 7, 'PO No.', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(20, 7, $DeliveryChallan['po_no'], 'B', false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(5, 7, '', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(20, 7, 'PO Date', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(40, 7, $DeliveryChallan['po_date'], 'B', false, 'L', 0, '', 0, false, 'M', 'M');
        
        $pdf->Ln(7);
        $pdf->Cell(30, 7, 'Terms Of Delivery', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(100, 7, $DeliveryChallan['terms_of_delivery'], 'B', false, 'L', 0, '', 0, false, 'M', 'M');

        $pdf->Cell(5, 7, '', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        
        $pdf->Cell(20, 7, 'Delivery Time', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(40, 7, date('h:i a', strtotime($DeliveryChallan['delivery_time'])), 'B', false, 'L', 0, '', 0, false, 'M', 'M');

        $pdf->Ln(7);
        $pdf->Cell(30, 7, 'Driver Name', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(165, 7, $DeliveryChallan['driver_name'], 'B', false, 'L', 0, '', 0, false, 'M', 'M');



        $pdf->Ln(7);
        $arrAddress = splitString($DeliveryChallan['delivery_address'],115);
        $pdf->Cell(30, 4, 'Delivery Address', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(165, 4, $arrAddress[0], 0, false, 'L', 0, '', 0, false, 'M', 'M');

        $pdf->Ln(4);
        $pdf->Cell(30, 4, '', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(165, 4, $arrAddress[1], 'B', false, 'L', 0, '', 0, false, 'M', 'M');
        ...
        $pdf->Cell(30,  7, 'Quantity', 1, false, 'C', 0, '', 0, false, 'M', 'M');
        $pdf->Ln(-.5);
        $pdf->Ln(1);
        ...
        foreach($details as $detail) {
            ...
            $productVendors = $this->model['delivery_challan_detail']->getProductVendorSerials($DeliveryChallan['delivery_challan_id'], $detail['product_master_id']);


            $productDesc = [];
            // $productDesc[] = $detail['brand'];
            // $productDesc[] = $detail['model'];
        ...
        $pdf->Ln(6);
        $pdf->Cell((195/2.1), 5, '', 'L', false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(9, 5, '', 0, false, 'R', 0, '', 0, false, 'M', 'M'); 
        $pdf->Cell((195/2.1), 5, 'E. & O. E', 'R', false, 'R', 0, '', 0, false, 'M', 'M');

        $pdf->Ln(5);
        $pdf->Cell((195/2.1), 5, 'Remarks:', 'TLR', false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(9, 5, '', 0, false, 'R', 0, '', 0, false, 'M', 'M'); 
        $pdf->Cell((195/2.1), 5, 'FOR ' . $session['company_branch_name'], 'TLR', false, 'L', 0, '', 0, false, 'M', 'M');




        $leftBox = $DeliveryChallan['remarks'];

        $rightBox = [];
        $rightBox[] = NULL;
        $rightBox[] = $DeliveryChallan['terms_of_delivery'];
        ...
        $pdf->SetTextColor(0,0,0);
        $pdf->MultiCell((195/2.1), 45, $leftBox, 'BLR', 'L', false, 0, '', ($Y+3), 'T', '', '');

        $pdf->Cell(172, 7, '', 0, false, 'C', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(30, 7, '', 'TLR', false, 'C', 0, '', 0, false, 'M', 'M');
        $pdf->Ln(7);
        $pdf->SetFont('times', 'B', 20);
        $pdf->MultiCell((195/2.1), 35, implode("\n", $rightBox), 'LR', 'C', false, 0, (195/2.1)+16, ($Y+3), 'M', '', '');
        ...
        $pdf->Ln(37.8);
        $pdf->SetFont('times', 'B', 9);
        $pdf->Cell((195/2.1)+9, 5, '', 0, false, 'C', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(((195/2.1)/2), 5, 'Syed Farhan Ali', 'BL', false, 'C', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(((195/2.1)/2), 5, 'Naeem Uddin', 'BR', false, 'C', 0, '', 0, false, 'M', 'M');

        $pdf->Ln(5);
        $pdf->Cell((195/2.1)+9, 5, '', 0, false, 'C', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(((195/2.1)/2), 5, 'Prepared By', 'BL', false, 'C', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(((195/2.1)/2), 5, 'Approved By', 'BR', false, 'C', 0, '', 0, false, 'M', 'M');


        $pdf->Ln(7);
        $pdf->Cell(195, 5, 'NOTE: CARTAGE SHOULD BE PAID BY THE CUSTOMER WHICH IS ___________________ RUPEES.', 0, false, 'C', 0, '', 0, false, 'M', 'M');
...
//        // Set font
        if($this->data['header']!=2){
            ...
            $this->Ln(8);
    //        // Title
            $this->Cell(0, 4, $this->data['company_branch_name'], 0, false, 'C', 0, '', 0, false, 'M', 'M');
            $this->Ln(9);
...
//        // Set font
        ...
        // $this->Ln(5);
        // $this->Cell(0, 5, 'Received the above goods in goods order and conditions', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        // $this->Ln(15);
        ...
        // $this->Cell(60, 5, 'For '.$this->data['company_name'], "T", false, 'C', 0, '', 0, false, 'M', 'M');
        // $this->Ln(10);

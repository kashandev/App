...
class ControllerInventoryPurchaseOrder extends HController {
    ...
    public function getAjaxLists() {
        ...
        $data = array();
        $aColumns = array('action', 'document_date','document_identity', 'partner_name', 'total_net_amount', 'invoice_type', 'created_at', 'check_box');
    ...
    protected function getForm() {
        ...
        if (isset($this->request->get['purchase_order_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
            ...
            $rows = $this->model['purchase_order_detail']->getRows(array('purchase_order_id' => $this->request->get['purchase_order_id']),array('sort_order asc'));
            // d($rows,true);
        ...
        $this->data['href_get_json_products_for_purchase_order'] = $this->url->link($this->getAlias() . '/getProductJsonForPurchaseOrder', 'token=' . $this->session->data['token'] . '&' . $this->getPrimaryKey() . '=' . $this->request->get[$this->getPrimaryKey()], 'SSL');
...
//        $filter[] = "LOWER(`p`.`tracing_type`) = 'traceable'";
    ...
    protected function insertData($data) {
        ...
        $data['base_amount'] = $data['total_net_amount'] * $data['conversion_rate'];
//        d($data,true);
        ...
        $insert_document = array(
            ...
            'document_amount' => $data['total_net_amount'],
    ...
    protected function updateData($primary_key, $data) {
        ...
        $insert_document = array(
            ...
            'conversion_rate' => $data['conversion_rate'],
            'base_currency_id' => $data['base_currency_id'],
            'base_amount' => ($data['total_net_amount'] * $data['conversion_rate']),
        );
        $document_id = $this->model['document']->add($this->getAlias(), $insert_document);
...
//    }
    ...
    public function printDocument() {
        ...
        $this->model['currency'] = $this->load->model('setup/currency');
        $currency = $this->model['currency']->getRow(array('company_id' => $this->session->data['company_id'], 'currency_id' => $voucher['document_currency_id']));
        ...
        $pdf->data = array(
            ...
            'terms'             => $voucher['terms'],
            'remarks'           => $voucher['remarks'],
            'document_currency' => $currency['currency_code'],
            'invoice_type'      => $voucher['invoice_type']

        );
        // set margins
        //$pdf->SetMargins(PDF_MARGIN_LEFT, PDF_MARGIN_TOP, PDF_MARGIN_RIGHT);
        $pdf->SetMargins(5, 84, 5);
        ...
        foreach($details as $detail) {
            ...
            // $project = ($detail['project'].' - '.$detail['sub_project']);
            $product_name = ($detail['product_code'].' - '.$detail['product_name']);
            if( strlen($product_name) <= 50 )
            {
                // $pdf->Cell(30, 6, $project, 1, false, 'L', 1, '', 1);
                $pdf->Cell(75, 6, $product_name, 1, false, 'L', 1, '', 1);
                $pdf->Cell(13, 6, $detail['unit'], 1, false, 'C', 1, '', 1);
                $pdf->Cell(20, 6, number_format($detail['qty'],2), 1, false, 'C', 1, '', 1);
                $pdf->Cell(20, 6, number_format($detail['rate'],2), 1, false, 'R', 1, '', 1);
                $pdf->Cell(25, 6, number_format($detail['amount'],2), 1, false, 'R', 1, '', 1);
                $pdf->Cell(22, 6, number_format($detail['tax_amount'],2), 1, false, 'R', 1, '', 1);
                $pdf->Cell(25, 6, number_format($detail['net_amount'],2), 1, false, 'R', 1, '', 1);
                $pdf->Ln(6);
            }
            else
            {
                // $arrProject = splitString($project, 27);
                $arrProduct = splitString($product_name, 50);

                foreach ($arrProduct as $index => $product) {
                    if( $index == 0 )
                    {
                        // $pdf->Ln(-6);      
                        // $pdf->Cell(30, 6, $arrProject[$index], 'LRT', false, 'L', 1, '', 1);
                        $pdf->Cell(75, 6, $product, 'LRT', false, 'L', 1, '', 1);
                        $pdf->Cell(13, 6, $detail['unit'], 'LRT', false, 'C', 1, '', 1);
                        $pdf->Cell(20, 6, number_format($detail['qty'],2), 'LRT', false, 'C', 1, '', 1);
                        $pdf->Cell(20, 6, number_format($detail['rate'],2), 'LRT', false, 'R', 1, '', 1);
                        $pdf->Cell(25, 6, number_format($detail['amount'],2), 'LRT', false, 'R', 1, '', 1);
                        $pdf->Cell(22, 6, number_format($detail['tax_amount'],2), 'LRT', false, 'R', 1, '', 1);
                        $pdf->Cell(25, 6, number_format($detail['net_amount'],2), 'LRT', false, 'R', 1, '', 1);
                        $pdf->Ln(6);
                        // $pdf->ln(7);
                    }
                    ...
                    {
                        // $pdf->ln(7);
                        // $pdf->Cell(30, 6, $arrProject[$index], 'LRB', false, 'L', 1, '', 1);
                        $pdf->Cell(75, 6, $product, 'LRB', false, 'L', 1, '', 1);
                        $pdf->Cell(13, 6, '', 'LRB', false, 'C', 1, '', 1);
                        $pdf->Cell(20, 6, '', 'LRB', false, 'C', 1, '', 1);
                        $pdf->Cell(20, 6, '', 'LRB', false, 'C', 1, '', 1);
                        $pdf->Cell(25, 6, '', 'LRB', false, 'C', 1, '', 1);
                        $pdf->Cell(22, 6, '', 'LRB', false, 'C', 1, '', 1);
                        $pdf->Cell(25, 6, '', 'LRB', false, 'C', 1, '', 1);
                        $pdf->ln(6);
                    }
                    else
                    {
                        // $pdf->ln(-7);
                        // $pdf->Cell(30, 6, $arrProject[$index], 'LR', false, 'L', 1, '', 1);
                        $pdf->Cell(75, 6, $product, 'LR', false, 'L', 1, '', 1);
                        $pdf->Cell(13, 6, '', 'LR', false, 'C', 1, '', 1);
                        $pdf->Cell(20, 6, '', 'LR', false, 'C', 1, '', 1);
                        $pdf->Cell(20, 6, '', 'LR', false, 'C', 1, '', 1);
                        $pdf->Cell(25, 6, '', 'LR', false, 'C', 1, '', 1);
                        $pdf->Cell(22, 6, '', 'LR', false, 'C', 1, '', 1);
                        $pdf->Cell(25, 6, '', 'LR', false, 'C', 1, '', 1);
                        $pdf->ln(6);
                    }
                }
        ...
        for ($i = $y; $i <= 200; $i++) {
         ...
         // $pdf->Cell(30, 6, '', 'LR', false, 'L', 1, '', 1);
            $pdf->Cell(75, 6, '', 'LR', false, 'L', 1, '', 1);
            $pdf->Cell(13, 6, '', 'LR', false, 'C', 1, '', 1);
            $pdf->Cell(20, 6, '', 'LR', false, 'C', 1, '', 1);
            $pdf->Cell(20, 6, '', 'LR', false, 'C', 1, '', 1);
            $pdf->Cell(25, 6, '', 'LR', false, 'C', 1, '', 1);
        ...
        // $pdf->Ln(-3);
        $pdf->ln(2);
        $pdf->Cell(199, 6, '', 'B', false, 'C', 0, '', 0, false, 'M', 'M');
        $x = $pdf->GetX();
        $y = $pdf->GetY();
        $pdf->setXY($x,$y);
        $pdf->SetFont('Helvetica', 'B', 8);
        $pdf->ln(3);
        $pdf->Cell(128, 7, 'IN WORD: ' . Number2Words(round($voucher['total_net_amount'],2)). ' only', 0, false, 'L');
        $pdf->Cell(25, 7, 'Total Amount', 1, false, 'L', 1, '', 1);
        $pdf->Cell(47, 7, number_format($voucher['total_amount'],2), 1, false, 'R', 1, '', 1);
        $pdf->ln(7);
        $pdf->Cell(128, 7, '', 0, false, 'L');
        $pdf->Cell(25, 7, 'Tax Amount', 1, false, 'L', 1, '', 1);
        $pdf->Cell(47, 7, number_format($voucher['total_tax_amount'],2), 1, false, 'R', 1, '', 1);
        $pdf->ln(7);
        $pdf->Cell(128, 7, '', 0, false, 'L');
        $pdf->Cell(25, 7, 'Net Amount', 1, false, 'L', 1, '', 1);
        $pdf->Cell(47, 7, number_format($voucher['total_net_amount'],2), 1, false, 'R', 1, '', 1);
...
class PDF extends TCPDF {
    ...
    public function Header() {
        ...
        // $this->Cell(134, 7, '', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        // $this->Cell(30, 7, 'Invoice Type: ', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        // $this->Cell(35, 7, $this->data['invoice_type'], 'B', false, 'L', 0, '', 0, false, 'M', 'M');

        // $this->Ln(10);
        $this->SetFont('Helvetica', '', 9);
        $this->Cell(25, 7, 'Document No. ', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $this->Cell(40, 7, $this->data['document_identity'], 'B', false, 'L', 0, '', 0, false, 'M', 'M');
        $this->Cell(69, 7, '', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $this->Cell(30, 7, 'Document Date: ', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $this->Cell(35, 7, stdDate($this->data['document_date']), 'B', false, 'L', 0, '', 0, false, 'M', 'M');

        $this->ln(10);
        $this->Cell(25, 7, 'Supplier Name: ', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $this->Cell(109, 7, html_entity_decode($this->data['partner_name']), 'B', false, 'L', 0, '', 0, false, 'M', 'M');
        $this->Cell(30, 7, 'Manual Ref No. ', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $this->Cell(35, 7, $this->data['manual_ref_no'], 'B', false, 'L', 0, '', 0, false, 'M', 'M');

        $this->ln(10);
        $this->Cell(17, 7, 'Terms: ', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $this->Cell(117, 7, $this->data['terms'], 'B', false, 'L', 0, '', 0, false, 'M', 'M');

        $this->Cell(30, 7, 'Document Currency: ', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $this->Cell(35, 7, $this->data['document_currency'], 'B', false, 'L', 0, '', 0, false, 'M', 'M');

        $this->ln(8);
        $this->Cell(17, 7, 'Remarks: ', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $this->Cell(182, 7, $this->data['remarks'], 'B', false, 'L', 0, '', 0, false, 'M', 'M');
        $this->ln(7);

        $this->SetFont('Helvetica', 'B', 8);
        $this->SetFillColor(255, 255, 255);
        $this->SetTextColor(0, 0, 0);
        // $this->Cell(30, 8, 'Project', 1, false, 'C', 1, '', 1);
        $this->Cell(75, 8, 'Product', 1, false, 'C', 1, '', 1);
        $this->Cell(13, 8, 'S/Unit', 1, false, 'C', 1, '', 1);
        $this->Cell(20, 8, 'Qty', 1, false, 'C', 1, '', 1);
        $this->Cell(20, 8, 'Rate', 1, false, 'C', 1, '', 1);
        $this->Cell(25, 8, 'Amount', 1, false, 'C', 1, '', 1);
        $this->Cell(22, 8, 'Tax Amount', 1, false, 'C', 1, '', 1);
        $this->Cell(25, 8, 'Net Amount', 1, false, 'C', 1, '', 1);
//        $this->ln(20);

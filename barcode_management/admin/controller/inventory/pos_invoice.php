...
//        $this->data['products'] = $this->model['product']->getRows(array('company_id' => $this->session->data['company_id']));
        ...
        $product_categories = $this->model['product_category']->getRows(array('company_id' => $this->session->data['company_id']));
        foreach($product_categories as $product_category) {
            if ($product_category['image'] && file_exists(DIR_IMAGE . $product_category['image']) && is_file(DIR_IMAGE . $product_category['image'])) {
                $image = $this->model['image']->resize($product_category['image'], 100, 100);
            } else {
                $image = $this->model['image']->resize('no_image.jpg', 100, 100);
            }
        ...
        foreach($products as $product) {
            if ($product['image'] && file_exists(DIR_IMAGE . $product['image']) && is_file(DIR_IMAGE . $product['image'])) {
                $image = $this->model['image']->resize($product['image'], 50, 50);
            } else {
                $image = $this->model['image']->resize('no_image.jpg', 50, 50);
            }

            $stock = $this->model['stock_ledger']->getStock(array(
        ...
        if($this->data['warehouses']) {
            $this->data['warehouse_id'] = $this->data['warehouses'][0]['warehouse_id'];
        }
        ...
        $this->data['href_get_products_json'] = $this->url->link($this->getAlias() . '/getProductsJson', 'token=' . $this->session->data['token'] . '&' . $this->getPrimaryKey() . '=' . $this->request->get[$this->getPrimaryKey()], 'SSL');
    ...
    public function getProducts() {
        ...
        $html = '';
        $row_no = 1;
        foreach($products as $product_no => $product) {
            if ($product['image'] && file_exists(DIR_IMAGE . $product['image']) && is_file(DIR_IMAGE . $product['image'])) {
                $product_image = $this->model['image']->resize($product['image'], 75, 75);
            } else {
                $product_image = $this->model['image']->resize('no_image.jpg', 75, 75);
            }
            if($row_no==1) {
                $html .= '<div class="row">';
            }
            $html .= '<div class="col-sm-2 product">';
            $html .= '<a href="javascript:void(0);" data-product_id="'.$product['product_id'].'" data-product_code="'.$product['product_code'].'" data-name="'.$product['name'].'" data-unit_id="'.$product['unit_id'].'" data-rate="'.$product['sale_price'].'" data-toggle="tooltip" title="'.$product['name'].'" onclick="addProduct(this);">';
            $html .= '<img alt="'.$product['name'].'" src="'.$product_image.'" />';
            $html .= '<div>'.$product['name'].'</div>';
            $html .= '</a>';
            $html .= '</div>';
            if($row_no==6 || $product_no==count($products)-1) {
                $row_no=0;
                $html .= '</div>';
            }
            $row_no++;
...
//            $where .= " AND is_post=1";
    ...
    public function printDocument() {
        ...
        //d(array($pos_invoice_id, $invoice, $details), true);

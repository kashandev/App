...
class ControllerProductionProduction extends HController {
    ...
    public function getAjaxLists() {
        ...
        $data = array();
        $aColumns = array('action', 'document_date', 'document_identity', 'product_code', 'product_name', 'actual_quantity', 'created_at', 'check_box');
    ...
    protected function getForm() {
        ...
        if (isset($this->request->get['production_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
            ...
            // d($this->request->get['production_id']);
            // d($this->data['production_details'],true);

            $detailRate =0;
            $PRDetails = $this->data['production_details'];
            foreach($PRDetails as $Details){
                $detailRate += $Details['cog_rate'];
            }
            $this->data['amount'] = $detailRate;
            $this->data['rate'] = $detailRate;
        }

        $this->data['href_get_master_product_json'] = $this->url->link($this->getAlias() . '/getMasterProductJson', 'token=' . $this->session->data['token'] . '&' . $this->getPrimaryKey() . '=' . $this->request->get[$this->getPrimaryKey()], 'SSL');
        $this->data['href_get_detail_product_json'] = $this->url->link($this->getAlias() . '/getDetailProductJson', 'token=' . $this->session->data['token'] . '&' . $this->getPrimaryKey() . '=' . $this->request->get[$this->getPrimaryKey()], 'SSL');

        $this->data['href_get_master_product_by_code'] = $this->url->link($this->getAlias() . '/getMasterProductByCode', 'token=' . $this->session->data['token'] . '&' . $this->getPrimaryKey() . '=' . $this->request->get[$this->getPrimaryKey()], 'SSL');
        $this->data['href_get_master_product_by_id'] = $this->url->link($this->getAlias() . '/getMasterProductById', 'token=' . $this->session->data['token'] . '&' . $this->getPrimaryKey() . '=' . $this->request->get[$this->getPrimaryKey()], 'SSL');

        $this->data['href_get_detail_product_by_code'] = $this->url->link($this->getAlias() . '/getDetailProductByCode', 'token=' . $this->session->data['token'] . '&' . $this->getPrimaryKey() . '=' . $this->request->get[$this->getPrimaryKey()], 'SSL');
        $this->data['href_get_detail_product_by_id'] = $this->url->link($this->getAlias() . '/getDetailProductById', 'token=' . $this->session->data['token'] . '&' . $this->getPrimaryKey() . '=' . $this->request->get[$this->getPrimaryKey()], 'SSL');
    ...
    public function getMasterProductJson() {
        $search = $this->request->post['q'];
        $page = $this->request->post['page'];
        $this->model[$this->getAlias()] = $this->load->model($this->getAlias());
        $rows = $this->model[$this->getAlias()]->getMasterProductJson($search, $page);

        echo json_encode($rows);
    }

    public function getDetailProductJson() {
        $search = $this->request->post['q'];
        $page = $this->request->post['page'];
        $this->model[$this->getAlias()] = $this->load->model($this->getAlias());
        $rows = $this->model[$this->getAlias()]->getDetailProductJson($search, $page);

        echo json_encode($rows);
    }

    public function getMasterProductByCode()
    {
    ...
    public function getMasterProductById()
    {
        $lang = $this->load->language('inventory/product');
        $post = $this->request->post;
        $where = [];
    ...
    public function getDetailProductByCode()
    {
        ...
        $where[] = "`company_id` = '". $this->session->data['company_id'] ."'";
        $where[] = "`product_code` = '". $post['product_code'] ."'";
        $where[] = "(`product_type` = 'Raw Material' OR `product_type` = 'Finished Goods' OR `product_type` = 'Services')";
        $where[] = "LOWER(`status`) = 'active'";
        $where = implode(' AND ', $where);
        $this->model['product'] = $this->load->model('inventory/product');
        $product = $this->model['product']->getRow($where);
    ...
    public function getBOM() {
        ...
        $rows = $this->model['bom_detail']->getRows(array('company_id' => $this->session->data['company_id'], 'master_product_id' => $post['product_id']), array('sort_order asc'));
        // d($this->model['stock']);
        // d($rows,true);
        $html = '';
        foreach($rows as $row_id => $row) {
            ...
            $html .= '<td style="width: 100px !important;">';
            $html .= '<select class="form-control warehouse" id="production_detail_warehouse_id_'.$row_id.'" name="production_details['.$row_id.'][warehouse_id]">';
            ...
            $html .= '<td hidden="hidden"><input type="text" class="form-control text-right" id="production_detail_cog_rate_'.$row_id.'" name="production_details['.$row_id.'][cog_rate]" value="'.$stock['avg_stock_rate'].'" readonly="true"/></td>';

            $html .= '<td hidden="hidden"><input type="text" class="form-control text-right" id="production_detail_cog_amount_'.$row_id.'" name="production_details['.$row_id.'][cog_amount]" value="'.$stock['cost_price'].'" readonly="true" /></td>';
            $html .= '</tr>';
        }

        // ($stock['avg_cogs_rate']<0)?$stock['cost_price']:$stock['avg_cogs_rate']
        // d($html,true);
        $json = array(
            'post' => $post,
            'stock' => $stock,
    ...
    protected function insertData($data) {
        ...
        foreach($stock_ledger_history as $sort_order => $ledger) {
            $ledger['company_id'] = $this->session->data['company_id'];
            $ledger['company_branch_id'] = $this->session->data['company_branch_id'];
            $ledger['fiscal_year_id'] = $this->session->data['fiscal_year_id'];
            $ledger['document_type_id'] = $this->document_type_id;
            ...
            $stock_ledger_history_id = $this->model['stock_ledger_history']->add($this->getAlias(), $ledger);
        }
        return $production_id;
    }

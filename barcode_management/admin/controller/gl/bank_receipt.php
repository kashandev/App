...
//        );
    ...
    protected function insertData($data) {
        ...
        foreach ($data['bank_receipt_details'] as $sort_order => $detail) {
            ...
            $gl_data[] = array(
                ...
                'document_credit' => 0,
                'document_debit' => $detail['net_amount'],
                'credit' => 0,
                'debit' => $detail['net_amount'],
                'its_no' => $data['its_no'],
        ...
        foreach($gl_data as $sort_order => $ledger) {
            ...
            $ledger['its_no'] = $data['its_no'];
            $ledger['challan_no'] = $data['challan_no'];
    ...
    protected function updateData($primary_key, $data) {
        ...
        $data['base_currency_id'] = 1;
        $data['base_currency'] = 'PKR';
        $data['document_currency_id'] = 1;
        ...
        foreach ($data['bank_receipt_details'] as $sort_order => $detail) {
            ...
            //this gl_data array placed here for
...
//        d(array($filter,$product_stock),true);
    ...
    public function printDocument() {
        ...
        $document_type_id = 9;
        $ledger_data = $this->model['bank_receipt']->getDataFromLedger($document_type_id, $bank_receipt_id);
        ...
        $voucher = $this->model['bank_receipt']->getRow(array('bank_receipt_id' => $bank_receipt_id));

        $details = $this->model['bank_receipt_detail']->getRows(array('bank_receipt_id' => $bank_receipt_id));
        ...
        $pdf->SetSubject('Bank Receipt');

        //Set Header
        $pdf->data = array(
            'company_name' => $session['company_name'],
            //'report_name' => $lang['heading_title'],
            'report_name' => 'Bank Receipt',
        ...
        // set font
        $pdf->SetFont('helvetica', '', 11, '', 'false');
        $pdf->Cell(30, 7, stdDate($voucher['document_date']), 'B', false, 'C', 0, '', 0, false, 'M', 'M');
        // set font
        ...
        $pdf->Cell(20, 7, 'ITS No: ', 0, false, 'R', 0, '', 0, false, 'M', 'M');
        // set font

        $pdf->SetFont('helvetica', '', 11, '', 'false');

        $pdf->Cell(40, 7, $voucher['its_no'], 'B', false, 'C', 0, '', 0, false, 'M', 'M');
        ...
        // set font
        $pdf->Cell(15, 7, '', 0, false, 'R', 0, '', 0, false, 'M', 'M');


        $pdf->SetFont('helvetica', '', 11, '', 'false');

        $pdf->Cell(30, 7, $voucher['challan_no'], 'B', false, 'C', 0, '', 0, false, 'M', 'M');
        ...
        $pdf->SetFont('courier', '', 11, '', 'false');
        $pdf->Cell(20, 7, 'Amaanat No: ', 0, false, 'R', 0, '', 0, false, 'M', 'M');
        // set font
        $pdf->SetFont('helvetica', '', 11, '', 'false');
        $pdf->Cell(40, 7, $details[0]['amaanat_no'], 'B', false, 'C', 0, '', 0, false, 'M', 'M');

        $pdf->Cell(55, 7, '', 0, false, 'R', 0, '', 0, false, 'M', 'M');

        $pdf->SetFont('courier', '', 11, '', 'false');

        $pdf->Cell(20, 7, 'Cheque No: ', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        // set font
        $pdf->Cell(15, 7, '', 0, false, 'R', 0, '', 0, false, 'M', 'M');
        ...
        $pdf->Cell(30, 7, $details[0]['cheque_no'], 'B', false, 'C', 0, '', 0, false, 'M', 'M');
        ...
        $pdf->SetFont('courier', '', 11, '', 'false');
        $pdf->Cell(20, 7, 'Sub Project: ', 0, false, 'R', 0, '', 0, false, 'M', 'M');
        // set font

        $pdf->SetFont('helvetica', '', 11, '', 'false');

        $pdf->Cell(70, 7, $voucher['sub_project_name'], 'B', false, 'C', 0, '', 0, false, 'M', 'M');
        ...
        $pdf->ln(11);
        $arrCoas = $this->model['coa']->getRow( array('coa_level3_id' => $details['coa_id']));
        ...
        $pdf->Cell(40, 8, 'Dr.', 1, false, 'C', 1, '', 1);
        $pdf->Cell(40, 8, 'Cr.', 1, false, 'C', 1, '', 1);
        //$pdf->Cell(50, 8, 'Remarks', 1, false, 'C', 1, '', 1);
        ...
        foreach($ledger_data as $row) {
            ...
            // $pdf->Cell(15, 8, '  ', 0, false, 'L', 0, '', 1);
//            $pdf->ln(8);

            //  $pdf->Cell(110, 8, $row['account'], 1, false, 'L', 0, '', 0, false, 'M', 'M');
            ...
            // $pdf->Cell(15, 8, ' ', 0, false, 'L', 0, '', 1);
            //$pdf->Cell(110, 16, $row['account'], 'B', false, 'L', 0, '', 0, false, 'M', 'M');
        ...
        }        // $pdf->Cell(110, 16, '', 'B', false, 'L', 0, '', 0, false, 'M', 'M');
...
*/
        ...
        $pdf->Output('Bank Receipt - '.$voucher['document_identity'].'.pdf', 'I');
...
class PDF extends TCPDF {
    ...
    public function Header() {
        ...
        if($this->data['company_logo'] != '') {
            ...
            //$this->Image($image_file, 11, 5, 150, '', '', '', 'C', false, 300, '', false, false, 1, false, false, false);
            $x = 15;
            $y = 0;
            $w = 180;
            $h = 40;
        ...
        */
        ...
        $this->Ln(2);
        // Title
        $this->Cell(0, 11, $this->data['company_name'], 0, false, 'C', 0, '', 0, false, 'M', 'M');
        $this->Ln(11);

        $this->SetFont('courier', '', 12);

        $this->Cell(0, 11, 'B-93 Shabbirabad, Tipu Sultan Road Karachi, Pakistan', 0, false, 'C', 0, '', 0, false, 'M', 'M');
        $this->Ln(11);
        $this->SetFont('helvetica', '', 16);
        $this->Cell(0, 11, $this->data['report_name'], 0, false, 'C', 0, '', 0, false, 'M', 'M');
    }

...
class ControllerGlCashReceipt extends HController {
    ...
    protected function getForm() {
        ...
        $this->data['href_get_documents'] = $this->url->link($this->getAlias() . '/getDocuments', 'token=' . $this->session->data['token'] . '&' . $this->getPrimaryKey() . '=' . $this->request->get[$this->getPrimaryKey()], 'SSL');
...
//        d(array($filter,$product_stock),true);
    ...
    public function printDocument() {
        ...
        $voucher = $this->model['cash_receipt']->getRow(array('cash_receipt_id' => $cash_receipt_id));

        $details = $this->model['cash_receipt_detail']->getRows(array('cash_receipt_id' => $cash_receipt_id));

        $document_type_id = 13;
        $ledger_data = $this->model['cash_receipt']->getDataFromLedger($document_type_id, $cash_receipt_id);
        ...
        $pdf->SetSubject('Cash Receipt');

        //Set Header
        $pdf->data = array(
            'company_name' => $session['company_name'],
            //'report_name' => $lang['heading_title'],
            'report_name' => 'Cash Receipt',
        ...
        // set font
        $pdf->SetFont('times', '', 9);
        $pdf->Cell(20, 7, stdDate($voucher['document_date']), 'B', false, 'L', 0, '', 0, false, 'M', 'M');
        // set font
        $pdf->SetFont('times', 'B', 9);
        $pdf->Cell(30, 7, 'Reference No: ', 0, false, 'R', 0, '', 0, false, 'M', 'M');
        // set font
        $pdf->SetFont('times', '', 9);
        $pdf->Cell(40, 7, $voucher['manual_ref_no'], 'B', false, 'C', 0, '', 0, false, 'M', 'M');
        $pdf->ln(10);
        ...
        $pdf->Cell(40, 7, '', 0, false, 'R', 0, '', 0, false, 'M', 'M');
        ...
        // set font
        $pdf->SetFont('times', '', 9);
        $pdf->Cell(60, 7, $voucher['sub_project_name'], 'B', false, 'C', 0, '', 0, false, 'M', 'M');
...
*/
        ...
        $pdf->MultiCell(150, 30, $voucher['remarks'], 1, 'L', 1, 0, '', '', true, 0, false, true, 40, 'T');
        ...
        $pdf->ln(10);
        $pdf->Cell(145, 7, '',  false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(35, 7, 'Receivers Signature : ', 0, false, 'C');



        //Close and output PDF document
        $pdf->Output('Cash Receipt - '.$voucher['document_identity'].'.pdf', 'I');

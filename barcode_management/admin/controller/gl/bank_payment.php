...
class ControllerGlBankPayment extends HController {
    ...
    public function getAjaxLists() {
        ...
        foreach ($results['lists'] as $aRow) {
            ...
            $actions[] = array(
                'text' => $lang['print_cheque'],
                'target' => '_blank',
                'href' => $this->url->link($this->getAlias() . '/printCheque', 'token=' . $this->session->data['token'] . '&' . $this->getPrimaryKey() . '=' . $aRow[$this->getPrimaryKey()], 'SSL'),
                'btn_class' => 'btn btn-info btn-xs',
                'class' => 'fa fa-print'
            );
...
//        $this->data['currencies'] = $this->model['currency']->getRows(array('company_id' => $this->session->data['company_id']));
        ...
        $this->data['qarze_hassana_requests'] = $this->model['qarze_hassana_request']->getRows(array('company_id' => $this->session->data['company_id']));
          ...
          //  d($result);
            foreach ($result as $field => $value) {
                if ($field == 'document_date') {
                    ...
                    $this->data[$field] = htmlentities($value);
                } else {
...
//        $this->data['href_get_documents'] = $this->url->link($this->getAlias() . '/getDocuments', 'token=' . $this->session->data['token']);
        ...
        $this->data['action_print_cheque'] = $this->url->link($this->getAlias() . '/printCheque', 'token=' . $this->session->data['token'] . '&' . $this->getPrimaryKey() . '=' . $this->request->get[$this->getPrimaryKey()], 'SSL');
        $this->data['action_print_cheque_summit'] = $this->url->link($this->getAlias() . '/printChequeSummit', 'token=' . $this->session->data['token'] . '&' . $this->getPrimaryKey() . '=' . $this->request->get[$this->getPrimaryKey()], 'SSL');
        $this->data['action_print_cheque_hbl'] = $this->url->link($this->getAlias() . '/printChequeHBL', 'token=' . $this->session->data['token'] . '&' . $this->getPrimaryKey() . '=' . $this->request->get[$this->getPrimaryKey()], 'SSL');
        $this->data['action_print_cheque_alhabib'] = $this->url->link($this->getAlias() . '/printChequeAlHabib', 'token=' . $this->session->data['token'] . '&' . $this->getPrimaryKey() . '=' . $this->request->get[$this->getPrimaryKey()], 'SSL');
...
//        d($sub_project_id,true);
    ...
    public function getCoas(){
        ...
        $this->model['coa_level_3'] = $this->load->model('gl/coa_level3');
        $rows = $this->model['sub_project']->getRows(array('company_id' => $this->session->data['company_id'], 'project_id' => $project_id),array('name'));
...
//        );
        ...
        foreach ($data['bank_payment_details'] as $sort_order => $detail) {
            ...
            $gl_data[] = array(
                'coa_id' => $data['transaction_account_id'],
                'document_debit' => 0,
                'document_credit' => $detail['net_amount'],
                'base_currency_id' => $data['base_currency_id'],
                'conversion_rate' => $data['conversion_rate'],
                'debit' => 0,
                'credit' => $detail['net_amount'] ,
                'remarks' => $data['remarks'],
                'project_id' => $data['project_id'],
                'sub_project_id' => $data['sub_project_id'],
                'its_no' => $data['its_no'],
            ...
            $gl_data[] = array(
                ...
                'conversion_rate' => $data['conversion_rate'],
                'debit' => 0,
                'credit' => $detail['other_tax_amount'] ,
                'partner_type_id' => $data['partner_type_id'],
                'partner_id' => $data['partner_id'],
                'ref_document_type_id' => $detail['ref_document_type_id'],
                'ref_document_identity' => $detail['ref_document_identity'],
...
//       d($post,true);
    ...
    public function printDocument() {
        ...
        $document_type_id = 8;
        $ledger_data = $this->model['bank_payment']->getDataFromLedger($document_type_id, $bank_payment_id);
        ...
        // $arrCoas = $this->model['coa']->getArrays('coa_id','level3_display_name', array('company_id' => $this->session->data['company_id']));
        ...
        // set font
        $pdf->SetFont('courier', '', 11, '', 'false');
        $pdf->Cell(35, 7, 'Voucher Date: ', 0, false, 'R', 0, '', 0, false, 'M', 'M');
        // set font
        $pdf->SetFont('helvetica', '', 11);
        ...
        $pdf->Cell(25, 7, 'Cheque No: ', 0, false, 'R', 0, '', 0, false, 'M', 'M');
        // set font
        $pdf->SetFont('helvetica', '', 11);


            $pdf->Cell(25, 7, $details[0]['cheque_no'], 'B', false, 'C', 0, '', 0, false, 'M', 'M');


        $pdf->ln(10);

        $pdf->SetFont('courier', '', 11, '', 'false');
        $pdf->Cell(20, 7, 'Project: ', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        // set font
        $pdf->SetFont('helvetica', '', 11);
        $pdf->Cell(40, 7, $voucher['project_name'], 'B', false, 'C', 0, '', 0, false, 'M', 'M');

        $pdf->Cell(30, 7, '', 0, false, 'R', 0, '', 0, false, 'M', 'M');
        $pdf->ln(10);

        $pdf->SetFont('courier', '', 11, '', 'false');
        $pdf->Cell(32, 7, 'Sub Project: ', 0, false, 'R', 0, '', 0, false, 'M', 'M');
        
        // set font
        $pdf->SetFont('helvetica', '', 11);
        $pdf->Cell(150, 7, $voucher['sub_project_name'], 'B', false, 'L', 0, '', 0, false, 'M', 'M');
...
//                $pdf->Cell(100, 8, '', 'B', false, 'L', 0, '', 1);
//                $pdf->Cell(40, 8, '', 1, false, 'R', 0, '', 1);
//                $pdf->Cell(40, 8, '', 1, false, 'R', 0, '', 1);
...
//                $pdf->ln(8);
                ...
                $pdf->Cell(40, 8, number_format($row['debit'],2), 1, false, 'R', 0, '', 1);
                $pdf->Cell(40, 8, number_format($row['credit'],2), 1, false, 'R', 0, '', 1);
        ...
        /* if($voucher['total_wht_amount'] > 0) {
             $pdf->ln(6);
             $pdf->Cell(120, 6, '', 0, false, 'L');
             $pdf->Cell(40, 6, 'WHT Amount: ', 'L', false, 'L');
         ...
         if($voucher['total_other_tax_amount'] > 0) {
             $pdf->ln(6);
             $pdf->Cell(120, 6, '', 0, false, 'L');
             $pdf->Cell(40, 6, 'OTHER TAX Amount: ', 'L', false, 'L');
 ...
 */
        ...
        if($voucher['document_currency_id'] == $voucher['base_currency_id'] || $voucher['conversion_rate'] == 1) {
            $pdf->ln(15);
            $pdf->SetFont('helvetica', '',11);
            $pdf->Cell(20, 7, 'Cheque Name: ', 0, false, 'L', 0, '', 0, false, 'M', 'M');

            $pdf->Cell(10, 7, '', 0, false, 'L', 0, '', 0, false, 'M', 'M');
            $pdf->Cell(150, 7, html_entity_decode($voucher['cheque_name']), 'B', false, 'L', 0, '', 0, false, 'M', 'M');
            $pdf->ln(10);
            ...
            $pdf->Cell(160, 7, Number2Words(round($voucher['total_net_amount'],2)).' only', 'B', false, 'L', 0, '', 0, false, 'M', 'M');
        }
        else
        {
            $pdf->ln(6);
            $pdf->Cell(120, 6, '', 0, false, 'R');
            $pdf->Cell(40, 6, $lang['total_amount'].': ', 0, false, 'L');
       ...
       */
        ...
        $pdf->ln(10);
        $pdf->Cell(30, 7, 'Narration : ', 0, false, 'L', 0, '', 0, false, 'M', 'M');
    ...
    public function printCheque() {
        ...
        $pageLayout = array(167, 60); //  or array($height, $width)
        $pdf = new PDF('L', PDF_UNIT, $pageLayout, true, 'UTF-8', false);
...
//         $pdf = new PDF('L', PDF_UNIT, 'A6', true, 'UTF-8', false);
        ...
        // set margins
        $pdf->SetMargins(PDF_MARGIN_LEFT, PDF_MARGIN_TOP, PDF_MARGIN_RIGHT);
        $pdf->SetMargins(0, 1, 5);
        //$pdf->SetHeaderMargin(PDF_MARGIN_HEADER);
        //$pdf->SetFooterMargin(PDF_MARGIN_FOOTER);

        // add a page
        $pdf->AddPage();
        // set font   $pdf->SetFont('times', '', 10);
        $pdf->SetFont('times', '', 12);
        $pdf->Cell(5, 7, '', 0, false, 'L');

        $pdf->Cell(120, 7, '', 0, false, 'L');
        $pdf->Cell(40, 7, stdDate($voucher['document_date']), 0, false, 'L');
        $pdf->Ln(11);
        $pdf->SetFont('times', '', 10);

        $pdf->Cell(3, 7, '', 0, false, 'L');
        $pdf->Cell(110, 7, html_entity_decode($voucher['cheque_name']), 0, false, 'L');
        $pdf->Ln(8);
        $pdf->SetFillColor(255,255,255);
       // $pdf->MultiCell(70, 20, Number2Words($voucher['total_net_amount']), 0, 'L', 1, 0, '', '', true, 0, false, true, 40, 'T');
        $pdf->Cell(3, 7, '', 0, false, 'L');
        $pdf->setCellHeightRatio(2.1);
        $pdf->MultiCell(105, 14, Number2Words(round($voucher['total_net_amount'])).' Only', 0, 'L', 1, 0, '', '', true, 0, false, true, 40, 'T');
        $pdf->Cell(5, 7, '', 0, false, 'L');

        //$pdf->Cell(115, 7,Number2Words(round($voucher['total_net_amount'],2)).' only', 0, false, 'L');
        $pdf->Cell(45, 7, '= '.number_format($voucher['total_net_amount'],0).' /-', 0, false, 'C');

        // set auto page breaks

  //      $pdf->SetAutoPageBreak(TRUE, PDF_MARGIN_BOTTOM);

//        $pdf->Cell(80, 7, Number2Words(round($voucher['total_net_amount'],2)).' only',  false, 'L', 0, '', 0, false, 'M', 'M');
    ...
    public function printChequeSummit() {
        ini_set('max_execution_time',0);
        ini_set('memory_limit',-1);
...
//         $pdf = new PDF('L', PDF_UNIT, 'A6', true, 'UTF-8', false);
        ...
        $pdf->SetFont('times', '', 12);
        $pdf->Ln(4);
        $pdf->Cell(110, 7, '', 0, false, 'L');
        $pdf->Cell(40, 7, stdDate($voucher['document_date']), 0, false, 'L');
        $pdf->Ln(9);
        $pdf->SetFont('times', '', 10);

        $pdf->Cell(5, 7, '', 0, false, 'L');
        $pdf->Cell(110, 7, $voucher['cheque_name'], 0, false, 'L');
        $pdf->Ln(8);
        $pdf->SetFillColor(255,255,255);
        // $pdf->MultiCell(70, 20, Number2Words($voucher['total_net_amount']), 0, 'L', 1, 0, '', '', true, 0, false, true, 40, 'T');
        $pdf->Cell(5, 7, '', 0, false, 'L');
        $pdf->setCellHeightRatio(1.8);
        $pdf->MultiCell(100, 20, Number2Words(round($voucher['total_net_amount'])), 0, 'L', 1, 0, '', '', true, 0, false, true, 40, 'T');

        //$pdf->Cell(115, 7,Number2Words(round($voucher['total_net_amount'],2)).' only', 0, false, 'L');
        $pdf->Cell(55, 7, number_format($voucher['total_net_amount'],0), 0, false, 'C');
...
//        $pdf->Cell(80, 7, Number2Words(round($voucher['total_net_amount'],2)).' only',  false, 'L', 0, '', 0, false, 'M', 'M');
    ...
    public function printChequeHBL() {
        ini_set('max_execution_time',0);
        ini_set('memory_limit',-1);
...
//        $pdf->Cell(80, 7, Number2Words(round($voucher['total_net_amount'],2)).' only',  false, 'L', 0, '', 0, false, 'M', 'M');
    ...
    public function printChequeAlHabib() {
        ini_set('max_execution_time',0);
        ini_set('memory_limit',-1);
...
//         $pdf = new PDF('L', PDF_UNIT, 'A6', true, 'UTF-8', false);
        ...
        $pdf->Cell(5, 7, '', 0, false, 'L');
        $pdf->setCellHeightRatio(1.8);
        $pdf->MultiCell(90, 21, Number2Words(round($voucher['total_net_amount'])).' only', 0, 'L', 1, 0, '', '', true, 0, false, true, 40, 'T');

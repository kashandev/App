...
//        d($BankReceiptDetails,true);
        ...
        $this->model['ledger'] = $this->load->model('gl/ledger');
        $LedgerDetails = $this->model['ledger']->getTransactionLedger(44,$BankReceiptId);
...
//        $pdf->Cell(60, 6, $BankReceipt['cost_center_name'], 0, false, 'L', 0, '', 0, false, 'M', 'M');
        ...
        foreach($LedgerDetails as $LedgerDetail) {
            if($LedgerDetail['debit'] > 0 || $LedgerDetail['credit'] > 0)
            {
                ...
                $pdf->Cell(105, 15, html_entity_decode($LedgerDetail['level3_name']), 0, 'L', 1, 0, '', '', true, 0, false, true, 40, 'T');
                ...
                {
                    $arrRemarks = splitString(html_entity_decode($LedgerDetail['remarks']), 110);
                    foreach($arrRemarks as $index => $remarks) {
                        if($index==0) {
                            $pdf->Cell(25, 6, '', 0, false, 'LB', 0, '', 1,false, 'T', 'T');
                            $pdf->Cell(105, 6,$remarks, 0, false, 'L', 0, '', 1,false, 'T', 'T');
                            $pdf->Cell(30, 6, '', 0, false, 'R', 0, '', 1,false, 'T', 'T');
                            $pdf->Cell(30, 6, '', 0, false, 'R', 0, '', 1,false, 'T', 'T');
                        }
                        elseif($index==count($arrRemarks)-1) {
                            $pdf->Ln(7);
                            $pdf->Cell(25, 6, '', 0, false, 'LB', 0, '', 1,false, 'T', 'T');
                            $pdf->Cell(100, 6, $remarks, 0, false, 'L', 0, '', 1,false, 'T', 'T');
                            $pdf->Cell(30, 6, '', 0, false, 'R', 0, '', 1,false, 'T', 'T');
                            $pdf->Cell(30, 6, '', 0, false, 'R', 0, '', 1,false, 'T', 'T');
                        }
                        else {

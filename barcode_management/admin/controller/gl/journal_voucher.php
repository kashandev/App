...
class ControllerGlJournalVoucher extends HController {
        ...
        if (isset($this->request->get['journal_voucher_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
           ...
           // $this->model['job_order'] = $this->load->model('production/job_order');
            ...
            foreach($details as $detail) {
                ...
                $detail['sub_projects'] = $this->model['sub_project']->getRows(['company_id' => $this->session->data['company_id'], 'project_id' => $detail['project_id']]);
    ...
    public function getPendingDocument() {
        ...
        $filter = array(
            ...
            'l.company_branch_id' => $session['company_branch_id'],
            'l.partner_type_id' => $post['partner_type_id'],
            'l.partner_id' => $post['partner_id'],
        );

        $filter['l.fiscal_year_id'] = $session['fiscal_year_id'];
        $documents = $this->model['document']->getPendingDocuments($filter);
        //d([$partner, $documents], true);
...
//        d(array($journal_voucher_id, $voucher, $details), true);
        ...
        $pdf->data = array(
            ...
            'ntn' => $company['ntn_no'],
            'strn' => ($session['company_branch_id']==20)?$company['gst_no']:'',

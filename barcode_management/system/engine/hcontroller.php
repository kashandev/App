...
//    protected $model = array();
    ...
    public function insert() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateInsert()) {
            ...
            $this->session->data['success'] = 'Information Saved Successfully';
    ...
    public function update() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateUpdate()) {
           ...
           // $this->session->data['success'] = $this->language->get('success_update');
            $this->session->data['success'] = 'Information Updated Successfully';
        ...
        if (isset($this->request->post['selected'])) {
            ...
            foreach ($this->request->post['selected'] as $id) {
                ...
                if($this->validateDelete($id)) {
                    $this->deleteData($id);
                    $this->session->data['success'] = 'Information Deleted Successfully';
                }
            }
            $this->db->commit();
    ...
    protected function getBreadCrumbs() {
        ...
        $arrRoutes = explode('/', $route);
        if(count($arrRoutes)==3)  {
            if($arrRoutes[2]=='update') {
                $this->data['breadcrumbs'][] = array(
                ...
                $this->data['breadcrumbs'][] = array(
                    'text' => $this->data['lang']['breadcrumb_insert'],
                    'href' => $this->url->link($this->getAlias().'/insert', 'token=' . $this->session->data['token'], 'SSL'),
                    'separator' => ' :: '
                );
            }
        }

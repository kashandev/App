...
$sql =
    ...
    "SELECT teams.uid FROM teams INNER JOIN orders on orders.uid = teams.uid WHERE teams.refcode = '" .
...
$sql =
    ...
    "SELECT orders.uid as touid FROM orders INNER JOIN teams on teams.uid = orders.uid WHERE teams.refcode = '" .
...
foreach ($tuid as $key => $thistuid) {
    ...
    if ($type == "com" && $search == "" || $search != "") {
        ...
        $thismsglabel = "There is no team commission found";

        $thisheading = "Team Commission";
   ...
   <?php foreach ($sql as $key => $row) {
       ...
       $paginationHtml .= "</tr>";
            ...
            $paginationHtml =
                ...
                '<div class="row"></div><div class="col-sm-12"><strong>There is no team commission found of ' .

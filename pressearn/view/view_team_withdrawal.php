...
echo '<h2 style="text-align: center;"><strong>Withdrawal</strong></h2>';
echo '<table class="table table-striped table-bordered all-table" id="table">';
echo '<thead><tr><th>User Name</th><th>Ref Code</th><th>Withdrawal Date</th><th>Withdrawal</th><th>Status</th></tr></thead><tbody>';

// Loop through each UID and fetch withdrawals
foreach ($tuid as $thistuid) {
    $all_qry = "SELECT users.uname, withdrawal.*, 
               (SELECT levelto FROM user_assign_level WHERE uid = users.userid LIMIT 1) AS downlinelv 
               FROM withdrawal 
               INNER JOIN users ON users.userid = withdrawal.uid 
               WHERE withdrawal.iswithdrawal = 1 AND withdrawal.uid = '$thistuid' 
               ORDER BY withdrawal.wtid DESC LIMIT $startFrom, $perPage";
    ...
    if ($totalRecords > 0) {
        while ($row = mysqli_fetch_assoc($res)) {
            ...
            $date = date_create($row["withdrawaldate"]);
        ...
        $message = 'There is no team withdrawal found';
    }
}

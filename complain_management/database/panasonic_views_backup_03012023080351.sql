...
FROM 
	(((((`core_stock_ledger` `sl` left join `in0_warehouse` `wh` on(`wh`.`company_id` = `sl`.`company_id` and `wh`.`company_branch_id` = `sl`.`company_branch_id` and `wh`.`warehouse_id` = `sl`.`warehouse_id`)) left join `in0_product` `p` on(`p`.`company_id` = `sl`.`company_id` and `p`.`product_id` = `sl`.`product_id`)) left join `in0_product_category` `pc` on(`pc`.`company_id` = `p`.`company_id` and `pc`.`product_category_id` = `p`.`product_category_id`)) left join `in0_unit` `ud` on(`ud`.`company_id` = `sl`.`company_id` and `ud`.`unit_id` = `sl`.`document_unit_id`)) left join `in0_unit` `ub` on(`ub`.`company_id` = `sl`.`company_id` and `ub`.`unit_id` = `sl`.`base_unit_id`));
...
SELECT 
	...
	`p`.`name` AS `product_name` 
FROM 
	(`ins_job_order` `jo` join `in0_product` `p` on(`p`.`product_id` = `jo`.`product_id`));
...
SELECT 
	...
	`p`.`name` AS `partner_name` 
FROM 
	(`ins_quotation` `dc` left join `core_partner` `p` on(`p`.`company_id` = `dc`.`company_id` and `p`.`company_branch_id` = `dc`.`company_branch_id` and `p`.`partner_type_id` = `dc`.`partner_type_id` and `p`.`partner_id` = `dc`.`partner_id`));

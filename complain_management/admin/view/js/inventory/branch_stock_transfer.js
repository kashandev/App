...
//    });
    ...
    $html += '</td>';
    $html += '<td>';
    $html += '<input type="text" onchange="calculateRowTotal(this);" class="form-control fPDecimal" name="branch_stock_transfer_details['+$grid_row+'][rate]" id="branch_stock_transfer_detail_rate_'+$grid_row+'" value="" />';
    ...
    $.ajax({
        ...
        success: function(json) {
            if(json.success)
            {
                ...
                $('#branch_stock_transfer_detail_cog_rate_'+$row_id).val(json.avg_stock_rate);
            }
            else {
                alert(json.error);
            }

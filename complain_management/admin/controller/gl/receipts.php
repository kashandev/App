...
//    }
    ...
    public function getDocuments() {
        ...
        {
            // Supplier
            $where .= " AND l.ref_document_type_id IN(1,3,7)";
        } else if( $partner_type_id == 2 ) {
            // Customer
            $where .= " AND l.ref_document_type_id = IN(7,23,39)";
        }
...
//d($Amount,true);
            foreach($Amount as  $ref_document_id => $amount)
            {
                ...
                $filter ['document_id'] = $ID['receipt_id'];
...
//        d(array($filter,$product_stock),true);
   ...
   public function printDocument() {
        ...
        $lang = $this->load->language($this->getAlias());
        $BankReceiptId = $this->request->get['receipt_id'];
        $post = $this->request->post;
        $session = $this->session->data;
        $this->model['company'] = $this->load->model('setup/company');
        $this->model['company_branch'] = $this->load->model('setup/company_branch');
        $this->model['receipt'] = $this->load->model('gl/receipts');
        $this->model['receipt_detail'] = $this->load->model('gl/receipts_detail');
        ...
        $BankReceipt = $this->model['receipt']->getRow(array('receipt_id' => $BankReceiptId));
        $TransactionAccount = $this->model['coa']->getRow(array('coa_level3_id' => $BankReceipt['transaction_account_id']));
        $BankReceiptDetails = $this->model['receipt_detail']->getRows(array('receipt_id' => $BankReceiptId));
        ...
        $pdf->data = array(
            ...
            'level3_name' => $TransactionAccount['level3_name'],
        ...
        $pdf->Cell(110, 7, $TransactionAccount['level3_name'], 'B', false, 'L', 0, '', 0, false, 'M', 'M');

        $pdf->Cell(5, 7, '', 0, false, 'L', 0, '', 0, false, 'M', 'M');

        $pdf->Cell(25, 7, 'Document No', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(30,7, $BankReceipt['document_identity'], 'B', false, 'L', 0, '', 0, false, 'M', 'M');
        
        $pdf->Ln(9);
        $pdf->Cell(30, 7, 'Partner Name', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(110, 7, $BankReceipt['partner_name'], 'B', false, 'L', 0, '', 0, false, 'M', 'M');
        
        $pdf->Cell(5, 7, '', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(25, 7, 'Document Date', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(30,7, stdDate($BankReceipt['document_date']), 'B', false, 'L', 0, '', 0, false, 'M', 'M');

        /*New*/
        $pdf->Ln(9);
        $pdf->Cell(15, 7, 'Project', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(43, 7, $BankReceipt['project_name'], 'B', false, 'L', 0, '', 0, false, 'M', 'M');


        $pdf->Cell(5.5, 7, '', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(25, 7, 'Sub Project', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(43, 7, $BankReceipt['sub_project_name'], 'B', false, 'L', 0, '', 0, false, 'M', 'M');

        //$pdf->Cell(5.5, 7, '', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        //$pdf->Cell(20, 7, 'Job Cart No.', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        //$pdf->Cell(43, 7, $BankReceipt['job_cart_name'], 'B', false, 'L', 0, '', 0, false, 'M', 'M');
        

        $pdf->Ln(10);
        $pdf->SetFont('helvetica', 'B', 8);
        $pdf->SetFillColor(255, 255, 255);
        $pdf->Cell(80, 7, 'Account', 1, false, 'C', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(20, 7, 'Cheque Date', 1, false, 'C', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(20, 7, 'Cheque No', 1, false, 'C', 1, '', 0, false, 'M', 'M');
        $pdf->Cell(20, 7, 'Total Amount', 1, false, 'C', 1, '', 0, false, 'M', 'M');
        $pdf->Cell(20, 7, 'WH Tax Amt', 1, false, 'R', 1, '', 0, false, 'M', 'M');
        $pdf->Cell(20, 7, 'OT Tax Amt', 1, false, 'R', 1, '', 0, false, 'M', 'M');
        $pdf->Cell(20, 7, 'Net Amount', 1, false, 'R', 1, '', 0, false, 'M', 'M');
        $sr = 0;
        $pdf->Ln(0);
        $Amount = 0;
        $WhAmount = 0;
        ...
        foreach($BankReceiptDetails as $detail) {
            ...
            $pdf->SetFont('helvetica', '', 8);
            $pdf->Cell(80, 7, html_entity_decode($Account['level3_name']), 1, false, 'L', 0, '', 0, false, 'M', 'M');
            $pdf->Cell(20, 7, (!empty($detail['cheque_date'])?stdDate($detail['cheque_date']):''), 1, false, 'C', 0, '', 0, false, 'M', 'M');
            $pdf->Cell(20, 7, $detail['cheque_no'], 1, false, 'C', 0, '', 0, false, 'M', 'M');
            $pdf->Cell(20, 7, number_format($detail['amount'],2), 1, false, 'R', 0, '', 0, false, 'M', 'M');
            $pdf->Cell(20, 7, number_format($detail['wht_amount'],2), 1, false, 'R', 0, '', 0, false, 'M', 'M');
            $pdf->Cell(20, 7, number_format($detail['other_tax_amount'],2), 1, false, 'R', 0, '', 0, false, 'M', 'M');
            $pdf->Cell(20, 7, number_format($detail['net_amount'],2), 1, false, 'R', 0, '', 0, false, 'M', 'M');
        }
        $x = $pdf->GetX();
        $y = $pdf->GetY();
        $pdf->setXY($x,$y);
        ...
        $pdf->Ln(7);
        $pdf->SetFont('helvetica', 'B', 8);
        $pdf->Cell(120, 7, 'Total', 1, false, 'R', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(20, 7, number_format($Amount,2), 1, false, 'R', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(20, 7, number_format($WhAmount,2), 1, false, 'R', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(20, 7, number_format($OtAmount,2), 1, false, 'R', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(20, 7, number_format($NetAmount,2), 1, false, 'R', 0, '', 0, false, 'M', 'M');

        // Previous Balance
        // $where[] = "l.document_identity != '{$BankReceipt['document_identity']}'";
        // $where[] = "l.partner_type_id = '{$BankReceipt['partner_type_id']}'";
        // $where[] = "l.partner_id = '{$BankReceipt['partner_id']}'";
        // $where[] = "l.document_date <= '{$BankReceipt['document_date']}'";
        // $where = 'WHERE ' . implode( ' AND ', $where );
        // $this->model['purchase_invoice'] = $this->load->model('inventory/purchase_invoice');
        // $previous_balance = $this->model['purchase_invoice']->getPreviousBalance($where, $BankReceipt['document_date']);



        // $pdf->Ln(7);
        // $pdf->Cell(120, 7, '', 0, false, 'R', 0, '', 0, false, 'M', 'M');
        // $pdf->Cell(60, 7, 'Previous Balance', 1, false, 'R', 0, '', 0, false, 'M', 'M');
        // if( $previous_balance < 0 )
        // {
        //     $pdf->Cell(20, 7, number_format(($previous_balance*-1),2), 1, false, 'R', 0, '', 0, false, 'M', 'M');
        // }
        // else
        // {
        //     $pdf->Cell(20, 7, number_format($previous_balance,2), 1, false, 'R', 0, '', 0, false, 'M', 'M');
        // }
        // $pdf->Ln(7);
        // $pdf->Cell(120, 7, '', 0, false, 'R', 0, '', 0, false, 'M', 'M');
        // $pdf->Cell(60, 7, 'Current Balance', 1, false, 'R', 0, '', 0, false, 'M', 'M');
        // $pdf->Cell(20, 7, number_format((($previous_balance-$NetAmount)),2), 1, false, 'R', 0, '', 0, false, 'M', 'M');
        ...
        $pdf->MultiCell(180, 15, html_entity_decode($BankReceipt['remarks']), 1, 'L', 1, 0, '', '', true, 0, false, true, 40, 'T');
...
//        // Set font
        ...
        // // Title
        // $this->Cell(0, 8, $this->data['company_name'], 0, false, 'C', 0, '', 0, false, 'M', 'M');
        // $this->Ln(8);
        // // $this->SetFont('helvetica', 'B', 14);
        // // $this->Cell(0, 7, $this->data['branch'], 0, false, 'C', 0, '', 0, false, 'M', 'M');
        // // $this->Ln(7);
        // $this->Cell(0, 7, $this->data['department'], 0, false, 'C', 0, '', 0, false, 'M', 'M');
        // $this->Ln(7);

        // $this->SetFont('helvetica', 'B', 12);
        // $this->Cell(0, 6, $this->data['report_name'], 0, false, 'C', 0, '', 0, false, 'M', 'M');

        // $this->Ln(10);

        // $this->SetFont('helvetica', '', 10);
        // $this->Cell(30, 7, 'Document No :', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        // $this->Cell(85,7, $this->data['document_identity'], 0, false, 'L', 0, '', 0, false, 'M', 'M');
        // $this->Cell(35, 7, 'Transaction Account :', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        // $this->Cell(70, 7, $this->data['level3_name'], 0, false, 'L', 0, '', 0, false, 'M', 'M');
        // $this->Ln(7);
        // $this->Cell(30, 7, 'Document Date :', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        // $this->Cell(85, 7, stdDate($this->data['document_date']), 0, false, 'L', 0, '', 0, false, 'M', 'M');
        // $this->Cell(15, 7, 'Partner :', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        // $this->Cell(70, 7, $this->data['partner_name'], 0, false, 'L', 0, '', 0, false, 'M', 'M');
    }
    ...
    public function Footer() {
        ...
        $this->Cell(8, 7, '', 0, false, 'C', 0, '', 0, false, 'M', 'M');

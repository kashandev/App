...
//       d($post,true);
     ...
     public function printDocument() {
        ...
        $lang = $this->load->language($this->getAlias());
        $BankPaymentId = $this->request->get['bank_payment_id'];
        $post = $this->request->post;
        ...
        $BankPayment = $this->model['bank_payment']->getRow(array('bank_payment_id' => $BankPaymentId));
        $TransactionAccount = $this->model['coa']->getRow(array('coa_level3_id' => $BankPayment['transaction_account_id']));
        $BankPaymentDetails = $this->model['bank_payment_detail']->getRows(array('bank_payment_id' => $BankPaymentId));
        ...
        $pdf->Cell(30,7, $BankPayment['document_identity'], 'B', false, 'L', 0, '', 0, false, 'M', 'M');
        
        $pdf->Ln(9);
        $pdf->Cell(30, 7, 'Partner Name', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(110, 7, $BankPayment['partner_name'], 'B', false, 'L', 0, '', 0, false, 'M', 'M');
        
        $pdf->Cell(5, 7, '', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(25, 7, 'Document Date', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(30,7, stdDate($BankPayment['document_date']), 'B', false, 'L', 0, '', 0, false, 'M', 'M');

        /*New*/
        $pdf->Ln(9);
        $pdf->Cell(15, 7, 'Project', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(43, 7, $BankPayment['project_name'], 'B', false, 'L', 0, '', 0, false, 'M', 'M');


        $pdf->Cell(5.5, 7, '', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(25, 7, 'SUb Project', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(43, 7, $BankPayment['sub_project_name'], 'B', false, 'L', 0, '', 0, false, 'M', 'M');

        $pdf->Cell(5.5, 7, '', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(20, 7, 'Job Cart', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(43, 7, $BankPayment['job_cart_name'], 'B', false, 'L', 0, '', 0, false, 'M', 'M');
        ...
        $NetAmount = 0;
        $pdf->SetFont('helvetica', '', 8);
        foreach($BankPaymentDetails as $detail) {
        ...
        // Previous Balance
        $where[] = "l.document_identity != '{$BankPayment['document_identity']}'";
        $where[] = "l.partner_type_id = '{$BankPayment['partner_type_id']}'";
        $where[] = "l.partner_id = '{$BankPayment['partner_id']}'";
        $where[] = "l.document_date <= '{$BankPayment['document_date']}'";
        $where = 'WHERE ' . implode( ' AND ', $where );
        $this->model['purchase_invoice'] = $this->load->model('inventory/purchase_invoice');
        $previous_balance = $this->model['purchase_invoice']->getPreviousBalance($where, $BankPayment['document_date']);
        ...
        $pdf->MultiCell(180, 15, html_entity_decode($BankPayment['remarks']), 1, 'L', 1, 0, '', '', true, 0, false, true, 40, 'T');

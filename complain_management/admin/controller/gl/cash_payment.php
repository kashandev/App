...
//        d(array($filter,$product_stock),true);
   ...
   public function printDocument() {
        ...
        $lang = $this->load->language($this->getAlias());
        $CashPaymentId = $this->request->get['cash_payment_id'];
        $post = $this->request->post;
        ...
        $CashPayment = $this->model['cash_payment']->getRow(array('cash_payment_id' => $CashPaymentId));
        $CashPaymentDetails = $this->model['cash_payment_detail']->getRows(array('cash_payment_id' => $CashPaymentId));
        $TransactionAccount = $this->model['coa']->getRow(array('coa_level3_id' => $CashPayment['transaction_account_id']));
        $Partner = $this->model['partner']->getRow(array('partner_id' => $CashPayment['partner_id']));
        ...
        $pdf->Cell(30,7, $CashPayment['document_identity'], 'B', false, 'L', 0, '', 0, false, 'M', 'M');
        
        $pdf->Ln(9);
        $pdf->Cell(30, 7, '', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(110, 7, '', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        
        $pdf->Cell(5, 7, '', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(25, 7, 'Document Date', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(30,7, stdDate($CashPayment['document_date']), 'B', false, 'L', 0, '', 0, false, 'M', 'M');

        /*New*/
        $pdf->Ln(9);
        $pdf->Cell(15, 7, 'Project', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(43, 7, $CashPayment['project_name'], 'B', false, 'L', 0, '', 0, false, 'M', 'M');


        $pdf->Cell(5.5, 7, '', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(25, 7, 'SUb Project', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(43, 7, $CashPayment['sub_project_name'], 'B', false, 'L', 0, '', 0, false, 'M', 'M');

        $pdf->Cell(5.5, 7, '', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(20, 7, 'Job Cart', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(43, 7, $CashPayment['job_cart_name'], 'B', false, 'L', 0, '', 0, false, 'M', 'M');


        $pdf->Ln(10);
        $pdf->SetFont('helvetica', 'B', 8);
        $pdf->Cell(80, 7, 'Account', 1, false, 'C', 0, '', 0, false, 'M', 'M');
        //$pdf->Cell(80, 7, 'Remarks', 1, false, 'C', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(30, 7, 'Total Amount', 1, false, 'C', 1, '', 0, false, 'M', 'M');
        $pdf->Cell(30, 7, 'WH Tax Amt', 1, false, 'C', 1, '', 0, false, 'M', 'M');
        $pdf->Cell(30, 7, 'OT Tax Amt', 1, false, 'C', 1, '', 0, false, 'M', 'M');
        $pdf->Cell(30, 7, 'Net Amount', 1, false, 'C', 0, '', 0, false, 'M', 'M');
        $sr = 0;
        $pdf->Ln(0);
        $NetAmount = 0;
        $pdf->SetFont('helvetica', '', 8);
        foreach($CashPaymentDetails as $detail) {
            ...
            $sr++;
            $pdf->Ln(7);
            $pdf->Cell(80, 7, $Account['level3_name'], 1, false, 'L', 0, '', 0, false, 'M', 'M');
             $pdf->Cell(30, 7, number_format($detail['amount'],2), 1, false, 'R', 0, '', 0, false, 'M', 'M');
            $pdf->Cell(30, 7, number_format($detail['wht_amount'],2), 1, false, 'R', 0, '', 0, false, 'M', 'M');
            $pdf->Cell(30, 7, number_format($detail['other_tax_amount'],2), 1, false, 'R', 0, '', 0, false, 'M', 'M');
        ...
        $pdf->Cell(30, 7, number_format($Amount,2), 1, false, 'R', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(30, 7, number_format($WhAmount,2), 1, false, 'R', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(30, 7, number_format($OtAmount,2), 1, false, 'R', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(30, 7, number_format($NetAmount,2), 1, false, 'R', 0, '', 0, false, 'M', 'M');
        
        // Previous Balance
        $where[] = "l.document_identity != '{$CashPayment['document_identity']}'";
        $where[] = "l.partner_type_id = '{$CashPayment['partner_type_id']}'";
        $where[] = "l.partner_id = '{$CashPayment['partner_id']}'";
        $where[] = "l.document_date <= '{$CashPayment['document_date']}'";
        $where = 'WHERE ' . implode( ' AND ', $where );
        $this->model['purchase_invoice'] = $this->load->model('inventory/purchase_invoice');
        $previous_balance = $this->model['purchase_invoice']->getPreviousBalance($where, $CashPayment['document_date']);
        ...
        $pdf->MultiCell(180, 15, html_entity_decode($CashPayment['remarks']), 1, 'L', 1, 0, '', '', true, 0, false, true, 40, 'T');

...
//       d($post,true);
     ...
     public function printDocument() {
        ...
        $lang = $this->load->language($this->getAlias());
        $PaymentsId = $this->request->get['payment_id'];
        $post = $this->request->post;
        $session = $this->session->data;
        $this->model['company'] = $this->load->model('setup/company');
        $this->model['company_branch'] = $this->load->model('setup/company_branch');
        $this->model['bank_payment'] = $this->load->model('gl/payments');
        $this->model['payments_detail'] = $this->load->model('gl/payments_detail');
        ...
        $Payments = $this->model['bank_payment']->getRow(array('payment_id' => $PaymentsId));
        $TransactionAccount = $this->model['coa']->getRow(array('coa_level3_id' => $Payments['transaction_account_id']));
        $PaymentsDetails = $this->model['payments_detail']->getRows(array('payment_id' => $PaymentsId));
        ...
        $pdf->Cell(30,7, $Payments['document_identity'], 'B', false, 'L', 0, '', 0, false, 'M', 'M');
        
        $pdf->Ln(9);
        $pdf->Cell(30, 7, 'Partner Name', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(110, 7, $Payments['partner_name'], 'B', false, 'L', 0, '', 0, false, 'M', 'M');
        
        $pdf->Cell(5, 7, '', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(25, 7, 'Document Date', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(30,7, stdDate($Payments['document_date']), 'B', false, 'L', 0, '', 0, false, 'M', 'M');

        /*New*/
        $pdf->Ln(9);
        $pdf->Cell(15, 7, 'Project', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(43, 7, $Payments['project_name'], 'B', false, 'L', 0, '', 0, false, 'M', 'M');


        $pdf->Cell(5.5, 7, '', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(25, 7, 'Sub Project', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(43, 7, $Payments['sub_project_name'], 'B', false, 'L', 0, '', 0, false, 'M', 'M');

        //$pdf->Cell(5.5, 7, '', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        //$pdf->Cell(20, 7, 'Job Cart No.', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        //$pdf->Cell(43, 7, $Payments['job_cart_name'], 'B', false, 'L', 0, '', 0, false, 'M', 'M');
        ...
        $NetAmount = 0;
        $pdf->SetFont('helvetica', '', 8);
        foreach($PaymentsDetails as $detail) {
            ...
            $sr++;
            $pdf->Ln(7);
            $cheque_date = (!(empty($detail['cheque_date']))?stdDate($detail['cheque_date']):'');
            $pdf->SetFont('helvetica', '', 8);
            $pdf->Cell(80, 7, html_entity_decode($Account['level3_name']), 1, false, 'L', 0, '', 0, false, 'M', 'M');
            $pdf->Cell(20, 7, $cheque_date, 1, false, 'C', 0, '', 0, false, 'M', 'M');
        ...
        // Previous Balance
        // $where[] = "l.document_identity != '{$Payments['document_identity']}'";
        // $where[] = "l.partner_type_id = '{$Payments['partner_type_id']}'";
        // $where[] = "l.partner_id = '{$Payments['partner_id']}'";
        // $where[] = "l.document_date <= '{$Payments['document_date']}'";
        // $where = 'WHERE ' . implode( ' AND ', $where );
        // $this->model['purchase_invoice'] = $this->load->model('inventory/purchase_invoice');
        // $previous_balance = $this->model['purchase_invoice']->getPreviousBalance($where, $Payments['document_date']);
        ...
        $pdf->MultiCell(180, 15, html_entity_decode($Payments['remarks']), 1, 'L', 1, 0, '', '', true, 0, false, true, 40, 'T');
        //Close and output PDF document
        $pdf->Output('Bank Payment:'.date('YmdHis').'.pdf', 'I');
    }

...
//d($json,true);
    ...
    protected function insertData($data) {
        ...
        foreach ($data['branch_stock_transfer_details'] as $sort_order => $detail) {
            ...
            $gl_data[] = array(
                'document_detail_id' => $branch_stock_transfer_detail_id,
                'coa_id' => $Frombranch['branch_account_id'],
...
//                'coa_id' => $product['adjustment_account_id'],
            ...
            if(floatval($balance) > 0 )
            {
                ...
                $gl_data[] = array(
                    'document_detail_id' => $branch_stock_transfer_detail_id,
                    'coa_id' => $pl_account_id,
                    'document_debit' => $BalanceAmount,
                    'document_credit' => 0,
                    'debit' => $BalanceAmount,
                    'credit' => 0,
                    'product_id' => $detail['product_id'],
                    'qty' => $detail['qty'],
                    'document_amount' => $BalanceAmount,
                    'amount' => ($BalanceAmount * $data['conversion_rate']),
                    'company_branch_id' => $this->session->data['company_branch_id'],
...
//                    'coa_id' => $product['adjustment_account_id'],
            ...
            // To Branch Account Entry //
            $gl_data[] = array(
                'document_detail_id' => $branch_stock_transfer_detail_id,
                'coa_id' => $ToBranch['branch_account_id'],
                'document_debit' => 0,
                'document_credit' => $detail['amount'],
                'debit' => 0,
                'credit' => $detail['amount'],
...
//                    'coa_id' => $product['adjustment_account_id'],
        ...
        //     d(array($data,$detail,$stock_ledger,$gl_data), true);
        foreach($stock_ledger as $sort_order => $ledger) {
            $ledger['company_id'] = $this->session->data['company_id'];
            $ledger['fiscal_year_id'] = $this->session->data['fiscal_year_id'];
            $ledger['document_type_id'] = $this->document_type_id;
...
//        } else {
    ...
    public function printDocument() {
        ...
        $pdf->Cell(10, 8, 'S.No.', 1, false, 'C', 1, '', 1);
        $pdf->Cell(60, 8, 'Item Description', 1, false, 'C', 1, '', 1);
        ...
        foreach($details as $detail) {
            ...
            $pdf->Cell(60, 8, html_entity_decode($detail['product_name']), 1, false, 'C', 0, '', 1);
            $pdf->Cell(25, 8, $Tobranch['name'], 1, false, 'C', 0, '', 1);
            $pdf->Cell(30, 8, $ToWarehouse['name'], 1, false, 'C', 0, '', 1);
            $pdf->Cell(15, 8, $detail['unit_code'], 1, false, 'C', 0, '', 1);

...
//                d($product,true);
                ...
                $html .= '<td>';
                $html .= '<input type="text" class="form-control" id="delivery_challan_detail'.$row_no.'_ref_document_identity" name="delivery_challan_details['.$row_no.'][ref_document_identity]" value="'.$sale_order['document_identity'].'" readonly/>
                <input type="hidden" value="'.$MasterData['sale_order_id'].'" name="delivery_challan_details['.$row_no.'][ref_document_id]" />';
                $html .= '<input type="hidden" class="form-control" id="delivery_challan_detail'.$row_no.'_ref_document_type_id" name="delivery_challan_details['.$row_no.'][ref_document_type_id]" value="'.$sale_order['document_type_id'].'" />';
                $html .= '</td>';

                $html .= '<td>';
                $html .= '<input type="text" class="form-control" id="delivery_challan_detail'.$row_no.'_product_code" name="delivery_challan_details['.$row_no.'][product_code]" value="'.$product['product_code'].'" readonly />';
                ...
                $html .= '</td>';
                $html .= '<td>';
                $html .= '<input type="hidden" class="form-control" id="delivery_challan_detail'.$row_no.'_product_id" name="delivery_challan_details['.$row_no.'][product_id]" value="'.$product['product_id'].'">';

                $html .= '<input type="text" class="form-control" id="delivery_challan_detail'.$row_no.'_product_name" name="delivery_challan_details['.$row_no.'][product_name]" value="'.html_entity_decode($product['product_name']).'" >';
                $html .= '</td>';

                $html .= '<td>';
                $html .= '<input type="hidden" class="form-control" id="delivery_challan_detail'.$row_no.'_document_identity" name="delivery_challan_details['.$row_no.'][cog_amount]" value="'.$product['cog_amount'].'" />';
                $html .= '<input type="text" class="form-control" id="delivery_challan_detail'.$row_no.'_description" name="delivery_challan_details['.$row_no.'][description]" value="'.htmlentities($product['description']).'" ></td>';

                $html .= '<td>';
                $html .= '<select type="text" class="form-control" id="delivery_challan_detail'.$row_no.'_warehouse_id" name="delivery_challan_details['.$row_no.'][warehouse_id]" >';
                $html .= ' <option value="">&nbsp;</option>';
                ...
                $html .= '</select>';
                $html .= '</td>';
                $html .= '<td><input type="text" onchange="calculateRowTotal(this)" class="form-control" id="delivery_challan_detail_qty_'.$row_no.'" name="delivery_challan_details['.$row_no.'][qty]" value="'.$balanceQty.'" ></td>';
                $html .= '<td><input type="text" class="form-control" id="delivery_challan_detail'.$row_no.'_unit" name="delivery_challan_details['.$row_no.'][unit]" value="'.$product['unit'].'" >';
                $html .= '<input type="hidden" class="form-control" id="delivery_challan_detail'.$row_no.'_unit_id" name="delivery_challan_details['.$row_no.'][unit_id]" value="'.$product['unit_id'].'" readonly></td>';
                $html .= '<td>';
                $html .= '<input type="text" class="form-control" id="delivery_challan_detail_cog_rate_'.$row_no.'" name="delivery_challan_details['.$row_no.'][cog_rate]" value="'.$product['rate'].'" readonly>';
                $html .= '<input type="hidden" class="form-control" id="delivery_challan_detail'.$row_no.'_cog_amount" name="delivery_challan_details['.$row_no.'][cog_amount]" value="'.$Cog_total.'" readonly>';
...
//d($json,true);
    ...
    protected function insertData($data) {
        ...
        // d($data['delivery_challan_details'], true);
...
if( $branch['company_branch_id'] == 18 ) {
// S.Nawabuddin & Co.         
$pdf->txt = $branch['address'].'
TEL: '. $branch['phone_no'] .'
connectnawab@gmail.com , www.sncpakistan.com
NTN: 0676943-8 STRN:12-01-9999-177-55';   
} else if( $branch['company_branch_id'] == 34 ) {
// Ayesha Trading Co.
$pdf->txt = $branch['address'].' TEL: '. $branch['phone_no'] .' NTN: 3980732-7';
} else if( $branch['company_branch_id'] == 35 ) {
// Ayesha Trading Co. (POS)
$pdf->txt = $branch['address'].' TEL: '. $branch['phone_no'] .', 0321-3963700 
PVC Suction Hose, Canvas Hose, Oil Delivery Hose, Hydraulic Hose, Pneumatic Hose , Hose Fittings Couplings';
} else if( $branch['company_branch_id'] == 36 ) {
// Abdullah Traders (POS)
$pdf->txt = $branch['address'].' TEL: '. $branch['phone_no'] .', 0321-3963700
PVC Suction Hose, Canvas Hose, Oil Delivery Hose, Hydraulic Hose, Pneumatic Hose , Hose Fittings Couplings';
} else if( $branch['company_branch_id'] == 37 ) {
// Abdullah Traders
$pdf->txt = $branch['address'].' TEL: '. $branch['phone_no'] .' NTN: 7216837-7';
}

        // set margins
        //$pdf->SetMargins(PDF_MARGIN_LEFT, PDF_MARGIN_TOP, PDF_MARGIN_RIGHT);
        $pdf->SetMargins(10, 65, 5);
        ...
        $pdf->SetFont('times', 'B', 10);
        $pdf->Cell(25, 7, 'Challan No :', 'B', false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(45, 7, $DeliveryChallan['document_identity'], 'B', false, 'L', 0, '', 0, false, 'M', 'M');
        ...
        $pdf->MultiCell(120 ,10, $Partner['address'], '', 'L', 1, 2, 10, 60, true);
        $pdf->Cell(5, 0, '', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->MultiCell(70, 10, 'Challan Date :' . stdDate($DeliveryChallan['document_date']), '', 'L', 1, 2, 135, 60, true);
        
        $pdf->Ln(1);
        $pdf->Cell(120, 7, '', '', false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(5, 7, '', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(25, 7, 'Po No :', 'B', false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(45, 7, $DeliveryChallan['po_no'], 'B', false, 'L', 0, '', 0, false, 'M', 'M');
        ...
        foreach($DeliveryChallanDetails as $detail) {
            ...
            $sr++;
            $pdf->Ln(7);
            $pdf->Cell(15, 7, $sr, 'L',  false, 'C', 0, '', 0, false, 'M', 'M');
            $pdf->Cell(120, 7, html_entity_decode($detail['description']), 'L', false, 'L', 0, '', 0, false, 'M', 'M');
            $pdf->Cell(30, 7, $detail['qty'], 'L', false, 'R', 0, '', 0, false, 'M', 'M');
            $pdf->Cell(30, 7, $detail['unit'], 'L,R', false, 'C', 0, '', 0, false, 'M', 'M');
        ...
        for ($i = $y; $i <= 220; $i++) {
            ...
            $pdf->Cell(120, 7, '', 'L', false, 'L', 0, '', 0, false, 'M', 'M');
            $pdf->Cell(30, 7, '', 'L', false, 'R', 0, '', 0, false, 'M', 'M');
            $pdf->Cell(30, 7, '', 'L,R', false, 'C', 0, '', 0, false, 'M', 'M');
            $y =$i;
        }

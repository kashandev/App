...
//    }
    ...
    public function printDocument() {
        ...
        $total_amount = 0;
        foreach($details as $detail) {
            $pdf->Cell(22, 8, $detail['product_code'], 'L', false, 'L', 0, '', 1);
            $pdf->Cell(90, 8, html_entity_decode($detail['product_name']), 'L', false, 'L', 0, '', 1);
            $pdf->Cell(15, 8, $detail['unit'], 'L', false, 'C', 0, '', 1);
            $pdf->Cell(19, 8, number_format($detail['qty'],2), 'L', false, 'R', 0, '', 1);
            $pdf->Cell(19, 8, number_format($detail['rate'],4), 'L', false, 'R', 0, '', 1);
            $pdf->Cell(24, 8, number_format($detail['amount'],2), 'L,R', false, 'R', 0, '', 1);
        ...
        $pdf->setXY($x,$y);
        $pdf->ln(9);
            $pdf->Cell(146, 6, 'IN WORD: ' . Number2Words(round($voucher['base_amount'],2)). ' only', 'L,B', false, 'L');

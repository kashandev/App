...
class ModelCommonStockLedger extends HModel {
    ...
    public function getStock($filter = array()) {
        $sql = "";
        $sql .= "SELECT SUM(base_qty) AS stock_qty, SUM(base_amount) AS stock_amount, base_rate as cost_price , ROUND(SUM(base_amount)/SUM(base_qty),2) AS avg_stock_rate";
        $sql .= " FROM `core_stock_ledger`";
        if($filter) {
            if(is_array($filter)) {
                $implode = array();
    ...
    public function getWarehouseStock($product_id, $warehouse_id='', $document_identity='', $document_date='') {
        $sql = "";
        $sql .= " SELECT IFNULL(SUM(base_qty),0) AS stock_qty, ROUND(IFNULL(SUM(base_amount)/SUM(base_qty),0),2) AS avg_stock_rate";
        $sql .= " FROM `core_stock_ledger`";

...
class ModelInventoryPurchaseOrder extends HModel {
    ...
    public function getPurchaseOrders($filter, $excl_document_id='') {
        ...
        foreach($purchase_orders as $purchase_order) {
            ...
            if(isset($arrPurchaseOrders[$purchase_order['document_identity']]['products'][$purchase_order['product_id']])) {
                $arrPurchaseOrders[$purchase_order['document_identity']]['products'][$purchase_order['product_id']]['order_qty'] += $purchase_order['qty'];
            } else {
                $arrPurchaseOrders[$purchase_order['document_identity']]['products'][$purchase_order['product_id']] = array(
            ...
            $rows = $query->rows;
            foreach($rows as $row) {
                $arrPurchaseOrders[$purchase_order['document_identity']]['products'][$row['product_id']]['utilized_qty'] += $row['qty'];
            }
        }
